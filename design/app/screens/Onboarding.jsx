...
// ── Step 1: Sign in (Google primary, Email → OTP secondary) ───
function SignInStep({ email, setEmail, onEmailNext, onGoogle }) {
    ...
    <div style={{ padding: '24px 24px 24px', display: 'flex', flexDirection: 'column', gap: 22, flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <h1 className="ck-display" style={{
          fontSize: 34, fontWeight: 700, margin: 0, lineHeight: 1.0, letterSpacing: '-0.035em',
        }}>
          Welcome to<br/>matchday<span style={{ color: 'var(--red)' }}>.</span>
        </h1>
        <p style={{ margin: '4px 0 0', color: 'var(--muted)', fontSize: 14.5, lineHeight: 1.45 }}>
          Cricket for the club, the village, the mohalla. Sign in to start.
        </p>
      </div>

      {/* Email + OTP — primary path */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <label className="ck-label" style={{ display: 'block', marginBottom: -2 }}>Email address</label>
        <input
          className="ck-input"
          type="email"
          inputMode="email"
      ...
      <div style={{
        ...
        color: 'var(--soft)', fontSize: 11, fontFamily: 'JetBrains Mono',
      ...
      {/* Google — secondary, at the bottom */}
      <button
        onClick={onGoogle}
        className="ck-btn"
        style={{
      ...
      <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--soft)', lineHeight: 1.5 }}>
        By continuing you agree to our <u>Terms</u> &amp; <u>Privacy Policy</u>.
      </div>
    </div>
  );
}

// ── Step 1 (legacy): Phone entry ──────────────────────────────
...
// ── Step 2: OTP (works for email or phone via `dest` prop) ────
function OtpStep({ dest, channel = 'email', otp, setOtp, onNext, onBack }) {
    ...
    <div style={{ padding: '32px 24px 24px', display: 'flex', flexDirection: 'column', gap: 28, flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <h1 className="ck-display" style={{ fontSize: 32, fontWeight: 700, margin: 0, lineHeight: 1.05 }}>
          Check your<br/>{channel === 'email' ? 'inbox' : 'SMS'}
        </h1>
        <p style={{ margin: 0, color: 'var(--muted)', fontSize: 15 }}>
          Sent to <span style={{ color: 'var(--ink)', fontWeight: 600 }}>{dest || 'you@example.com'}</span>
          {' · '}
...
function CkOnboarding() {
  ...
  const [email, setEmail] = React.useState('ahmed.khan@gmail.com');
  ...
  return (
    <div className="ck-screen" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '14px 24px 0' }}>
        {step > 0 && <StepDots index={Math.min(step - 1, 3)} total={4} />}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {step === 0 && (
          <SignInStep
            email={email}
            setEmail={setEmail}
            onEmailNext={() => setStep(1)}
            onGoogle={() => setStep(2)}
          />
        )}
        {step === 1 && <OtpStep dest={email} channel="email" otp={otp} setOtp={setOtp} onNext={() => setStep(2)} onBack={() => setStep(0)} />}
