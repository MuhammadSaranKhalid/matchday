...
function PvHome({ face, go }) {
  ...
  return (
    <div style={{ paddingBottom: 28 }}>
      ...
      <div style={{ padding: '0 18px', display: 'grid', gap: 6 }}>
        <PvRow icon="◉" title="My matches" sub={isNew ? 'No matches yet' : '2 requests · 3 confirmed · 142 past'} meta={isNew ? '0' : '2'} onClick={() => go('my-matches')} />
...
function PvMyMatches({ back, face }) {
  ...
  const incoming = [
    {
      id: 'in1',
      from: { name: 'Lahore Lions',  short: 'LAH', color: red },
      to:   { name: 'Karachi Eagles', short: 'KE', color: 'oklch(0.55 0.15 250)' },
      sender: 'Bilal Ahmed · captain',
      when: 'Sat 18 May · 15:00',
      venue: 'Model Town Ground',
      format: '20 ov · 11-a-side · white ball',
      age: '38m ago',
      expires: '23h',
      note: '"Rematch from May? Same ground, same time."',
    },
    {
      id: 'in2',
      from: { name: 'Old Boys', short: 'OB', color: 'oklch(0.36 0.04 80)' },
      to:   { name: 'Karachi Eagles', short: 'KE', color: 'oklch(0.55 0.15 250)' },
      sender: 'Faisal A. · captain',
      when: 'Sun 26 May · 07:00',
      venue: 'Iqbal Park',
      format: 'T10 · tape ball',
      age: '4h ago',
      expires: '2d',
    },
  ];
  const outgoing = [
    {
      id: 'out1',
      from: { name: 'Karachi Eagles', short: 'KE', color: 'oklch(0.55 0.15 250)' },
      to:   { name: 'Mohalla Kings',  short: 'MOH', color: 'oklch(0.78 0.14 80)' },
      sender: 'You · sent yesterday',
      when: 'Sat 25 May · 16:00',
      venue: 'Gulberg Sports',
      format: '20 ov · 11-a-side',
    ...
    },
    {
      id: 'out2',
      from: { name: 'Karachi Eagles', short: 'KE', color: 'oklch(0.55 0.15 250)' },
      to:   { name: 'DHA United',     short: 'DHA', color: 'oklch(0.42 0.10 260)' },
      sender: 'You · countered',
      when: 'Sun 19 May · 10:00',
      venue: 'DHA Ground · counter',
      format: '20 ov · 11-a-side',
  ...
  const confirmed = [
    {
      id: 'c1', tag: "Spring Cup '26 · QF",
      home: { name: 'Lahore Lions',  short: 'LAH', color: red, mine: true },
      away: { name: 'City Eagles',   short: 'CTY', color: 'oklch(0.42 0.10 260)' },
      when: 'Today · 18:30',
    ...
    {
      id: 'c2', tag: "Spring Cup · SF",
      home: { name: 'Lahore Lions',  short: 'LAH', color: red, mine: true },
      away: { name: 'DHA United',    short: 'DHA', color: 'oklch(0.42 0.10 260)' },
      when: 'Sat 18 · 15:00',
      venue: 'Gulberg Sports',
    ...
    },
    {
      id: 'c3', tag: 'Friendly',
      home: { name: 'Lahore Lions',  short: 'LAH', color: red, mine: true },
      away: { name: 'Practice',      short: 'INT', color: 'oklch(0.52 0.02 80)' },
      when: 'Sun 19 · 07:00',
      venue: 'Home ground',
      role: 'Optional',
      countdown: 'In 5 days',
  ...
  const past = [
    { id: 'p1', tag: 'Spring Cup · QF', when: 'Mon 10 May', home: { name: 'Lions', short: 'LAH', color: red, runs: 181, wickets: 6 }, away: { name: 'Eagles', short: 'CTY', color: 'oklch(0.42 0.10 260)', runs: 178, wickets: 10 }, winner: 'home', result: 'won by 3 runs', mine: 'You: 78 (52) · MOM' },
    { id: 'p2', tag: 'Friendly', when: 'Thu 6 May', home: { name: 'Lions', short: 'LAH', color: red, runs: 138, wickets: 8 }, away: { name: 'Old Boys', short: 'OB', color: 'oklch(0.36 0.04 80)', runs: 150, wickets: 6 }, winner: 'away', result: 'lost by 12', mine: 'You: 12 (15)' },
    { id: 'p3', tag: 'Friday League · R4', when: 'Fri 30 Apr', home: { name: 'Lions', short: 'LAH', color: red, runs: 142, wickets: 8 }, away: { name: 'Kings XI', short: 'KXI', color: 'oklch(0.78 0.14 80)', runs: 144, wickets: 4 }, winner: 'away', result: 'lost by 6 wkts', mine: 'You: 41 (28)' },
  ];
  ...
  const Subhead = ({ children, side, accent }) => (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '14px 0 8px' }}>
      <span style={{ ...monoLabel, color: accent || ink }}>{children}</span>
      {side && <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, letterSpacing: '0.06em' }}>{side}</span>}
    </div>
  );

  // ── Request cards ─────────────────────────────────────
  const IncomingCard = ({ rq }) => (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
        <PvPill kind="amber">Incoming</PvPill>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, letterSpacing: '0.06em' }}>
          {rq.age} · expires {rq.expires}
        </span>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <Crest s={rq.from.short} c={rq.from.color} size={28} />
        <div style={{ fontSize: 12, color: muted, fontFamily: 'JetBrains Mono', letterSpacing: '0.04em' }}>vs</div>
        <Crest s={rq.to.short} c={rq.to.color} size={28} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 13, color: ink, lineHeight: 1.25 }}>
            {rq.from.name} <span style={{ color: muted, fontWeight: 500 }}>wants to play</span> {rq.to.name}
          </div>
          <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>{rq.sender}</div>
        </div>
      </div>

      <div style={{
        marginTop: 10, padding: '8px 10px',
        background: paper2, borderRadius: 8,
        fontSize: 11, color: ink2, lineHeight: 1.5,
      }}>
        <div><b style={{ color: ink }}>{rq.when}</b> · {rq.venue}</div>
        <div style={{ color: muted, marginTop: 2 }}>{rq.format}</div>
        {rq.note && <div style={{ marginTop: 6, fontStyle: 'italic', color: ink2 }}>{rq.note}</div>}
      </div>

      <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
        <button style={{ flex: 1.2, padding: '9px 0', borderRadius: 8, border: 'none', background: ink, color: paper, fontSize: 12, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>Accept</button>
        <button style={{ flex: 1, padding: '9px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: ink, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>Counter</button>
        <button style={{ flex: 0.8, padding: '9px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: muted, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>Decline</button>
      </div>
    </div>
  );

  const OutgoingCard = ({ rq }) => (
    <div style={{
      background: paper, border: '1px solid ' + hair, borderRadius: 12,
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
        <PvPill>Outgoing · {rq.status.toLowerCase()}</PvPill>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted }}>sent {rq.age}</span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: 13, color: ink, lineHeight: 1.25 }}>
            You <span style={{ color: muted, fontWeight: 500 }}>challenged</span> {rq.to.name}
          </div>
          <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>{rq.sender}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
        <button style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: ink, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>View thread</button>
        <button style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: muted, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>Cancel</button>
      </div>
    </div>
  );

  const ConfirmedCard = ({ m }) => (
    <button style={{
      width: '100%', textAlign: 'left', fontFamily: 'inherit', cursor: 'pointer',
      background: paper, border: '1px solid ' + (m.urgent ? red : hair),
      borderRadius: 12, padding: 0, overflow: 'hidden',
    }}>
      <div style={{
        padding: '8px 14px', borderBottom: '1px solid ' + hair,
        background: paper2, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700, color: m.urgent ? red : ink, letterSpacing: '0.1em' }}>
          {m.when.toUpperCase()}
        </span>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, letterSpacing: '0.06em' }}>{m.tag}</span>
      </div>
      <div style={{ padding: '12px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Crest s={m.home.short} c={m.home.color} size={28} />
          <div style={{ fontSize: 12, color: muted, fontFamily: 'JetBrains Mono' }}>vs</div>
          <Crest s={m.away.short} c={m.away.color} size={28} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 600, fontSize: 13, color: ink }}>{m.home.name} vs {m.away.name}</div>
            <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>{m.venue}</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, paddingTop: 8, borderTop: '1px dashed ' + hair }}>
          <span style={{
            fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700,
            color: m.role.startsWith('Captain') ? red : ink, letterSpacing: '0.06em',
          }}>{m.role.startsWith('Captain') ? '✦ ' : ''}{m.role}</span>
          <span style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: m.urgent ? red : muted, fontWeight: 700, letterSpacing: '0.06em' }}>{m.countdown}</span>
        </div>
      </div>
    </button>
  );
  ...
  const PastRow = ({ m }) => (
    <button style={{
      ...
      <div style={{ minWidth: 56 }}>
        <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700, color: ink, letterSpacing: '0.06em' }}>{m.when.toUpperCase()}</div>
        <div style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, marginTop: 2 }}>{m.tag}</div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <Crest s={m.home.short} c={m.home.color} size={22} />
        <Crest s={m.away.short} c={m.away.color} size={22} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: ink, fontVariantNumeric: 'tabular-nums' }}>
          {m.home.runs}/{m.home.wickets} <span style={{ color: muted, fontWeight: 500 }}>v</span> {m.away.runs}/{m.away.wickets}
        </div>
        <div style={{ fontSize: 10, color: muted, marginTop: 2, fontFamily: 'JetBrains Mono', letterSpacing: '0.04em' }}>
          {m.mine}
        </div>
      </div>
      <span style={{
        fontFamily: 'JetBrains Mono', fontSize: 9, fontWeight: 700, letterSpacing: '0.08em',
        color: m.winner === 'home' ? green : red,
      }}>{m.result.split(' ')[0].toUpperCase()}</span>
    </button>
  );
  ...
  return (
    <div>
      <PvHeader
        title="My matches"
        sub={`${counts.requests} requests · ${counts.confirmed} confirmed · ${counts.past} past`}
        onBack={back}
        right={
          <button style={{
            padding: '6px 10px', borderRadius: 8, border: '1px solid ' + hair,
            background: paper, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
            display: 'inline-flex', alignItems: 'center', gap: 4,
          }}>+ Challenge</button>
        }
      />
      ...
      <div style={{ padding: '12px 18px 6px' }}>
        <div style={{
          ...
          background: paper2, border: '1px solid ' + hair, borderRadius: 10,
        }}>
          {[
            { id: 'requests',  label: 'Requests',  badge: counts.requests, badgeAccent: amber },
            { id: 'confirmed', label: 'Confirmed', badge: counts.confirmed },
            { id: 'past',      label: 'Past',      badge: counts.past },
          ].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              flex: 1, padding: '7px 0', borderRadius: 7,
              background: tab === t.id ? paper : 'transparent',
              border: 'none', color: tab === t.id ? ink : muted,
              fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
              boxShadow: tab === t.id ? '0 1px 3px rgba(40,30,15,0.08)' : 'none',
              ...
              <span style={{
                ...
                background: t.badgeAccent && tab === t.id ? t.badgeAccent : (tab === t.id ? ink : 'transparent'),
                color: t.badgeAccent && tab === t.id ? 'oklch(0.30 0.02 80)' : (tab === t.id ? paper : muted),
              }}>{t.badge}</span>
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '4px 18px 24px' }}>
        {tab === 'requests' && (
          <>
            <Subhead side={`${incoming.length} need your reply`} accent={amber}>Incoming · {incoming.length}</Subhead>
            <div style={{ display: 'grid', gap: 10 }}>
              {incoming.map(rq => <IncomingCard key={rq.id} rq={rq} />)}
            </div>

            <Subhead side={`${outgoing.length} awaiting`}>Outgoing · {outgoing.length}</Subhead>
            <div style={{ display: 'grid', gap: 10 }}>
              {outgoing.map(rq => <OutgoingCard key={rq.id} rq={rq} />)}
            </div>

            <div style={{ marginTop: 16, padding: '12px 14px', background: paper2, border: '1px dashed ' + hair, borderRadius: 10, fontSize: 11, color: muted, lineHeight: 1.5 }}>
              <PvPill>Note</PvPill> Tournament fixtures and roster invites live in <b style={{ color: ink2 }}>Status</b>. Match requests = friendly challenges only.
            </div>
          </>
        )}
        ...
        {tab === 'confirmed' && (
          <>
            <Subhead side="upcoming · next 14 days">Confirmed · {confirmed.length}</Subhead>
            <div style={{ display: 'grid', gap: 10 }}>
              {confirmed.map(m => <ConfirmedCard key={m.id} m={m} />)}
            </div>
          </>
        )}
        ...
        {tab === 'past' && (
          <>
            <Subhead side="last 30 days">Past · {past.length} shown</Subhead>
            <div style={{ display: 'grid', gap: 6 }}>
              {past.map(m => <PastRow key={m.id} m={m} />)}
            </div>
            <button style={{
              width: '100%', marginTop: 12, padding: '10px 14px',
              background: 'transparent', border: '1px dashed ' + hair, borderRadius: 10,
              color: ink2, fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700,
              letterSpacing: '0.08em', cursor: 'pointer',
            }}>SEE ALL 142 MATCHES →</button>
          </>
        )}
      </div>
    </div>
...
function PvMyTeams({ back, empty }) {
  // ── EMPTY STATE ─────────────────────────────────────
  if (empty) {
    const pendingInvites = [
      { name: 'DHA United',  short: 'DHA', color: 'oklch(0.42 0.10 260)', from: 'Asad Q. · captain', age: '2h' },
      { name: 'Mohalla Kings', short: 'MOH', color: 'oklch(0.78 0.14 80)', from: 'Faisal A.',          age: '1d' },
    ];

    return (
      <div>
        <PvHeader title="My teams" sub="You haven't joined a team yet." onBack={back} />
        ...
        <div style={{ padding: '24px 18px 18px', textAlign: 'center' }}>
          ...
          <div style={{ ...display(22), marginBottom: 6 }}>Pick a side.</div>
          <div style={{ fontSize: 13, color: ink2, lineHeight: 1.5, maxWidth: 280, margin: '0 auto' }}>
            Your stats, fixtures and captain duties all live with a team.
            Join one, accept an invite, or start your own.
          </div>
        </div>

        {/* Pending invites — surfaces when any exist */}
        {pendingInvites.length > 0 && (
          <>
            <PvSectionH count={pendingInvites.length} accent={amber}>Waiting for you</PvSectionH>
            <div style={{ padding: '0 18px', display: 'grid', gap: 8 }}>
              {pendingInvites.map((t, i) => (
                <PvCard key={i} accent={amber}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                        <PvPill kind="amber">Invite</PvPill>
                        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted }}>{t.age}</span>
                      </div>
                      <div style={{ fontWeight: 700, fontSize: 14, fontFamily: 'Inter Tight', marginTop: 4 }}>{t.name}</div>
                      <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>From {t.from}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                    <button style={{ flex: 1.4, padding: '9px 0', borderRadius: 8, border: 'none', background: ink, color: paper, fontSize: 12, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>Accept</button>
                    <button style={{ flex: 1, padding: '9px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: ink, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>View team</button>
                    <button style={{ flex: 0.8, padding: '9px 0', borderRadius: 8, border: '1px solid ' + hair, background: paper, color: muted, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>Decline</button>
                  </div>
                </PvCard>
              ))}
        ...
        {/* Three paths */}
        <PvSectionH>Get on a team</PvSectionH>
          ...
          {[
            {
              ...
              t: 'Create a team',
              s: 'Name, crest, home ground, starting roster. 5 steps — invites go out at the end.',
              cta: 'New team',
              meta: 'For captains',
            },
            {
              accent: ink,
              icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg>,
              t: 'Find a team nearby',
              s: 'Browse clubs and village teams in Karachi. Filter by ball type, format, level.',
              cta: 'Find teams',
              meta: '24 within 50km',
            },
            {
              accent: amber,
              icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="6" width="18" height="13" rx="2"/><path d="M3 7l9 7 9-7"/></svg>,
              t: 'Have an invite code?',
              s: 'A captain sent you a join link or a 6-letter code. Enter it here.',
              cta: 'Enter code',
              meta: 'From a captain',
            },
          ].map((p, i) => (
            <button key={i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: ink, fontFamily: 'Inter Tight' }}>{p.t}</div>
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, letterSpacing: '0.08em' }}>{p.meta}</span>
                </div>
                <div style={{ fontSize: 12, color: ink2, lineHeight: 1.5, marginBottom: 10 }}>{p.s}</div>
                <span style={{
                  display: 'inline-flex', alignItems: 'center', gap: 4,
                  fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700,
                  letterSpacing: '0.08em', color: ink,
        ...
        <div style={{ padding: '0 18px 28px' }}>
          <div style={{
            ...
            <span style={{ ...monoLabel, color: muted, flexShrink: 0, marginTop: 1 }}>FYI</span>
            <div style={{ flex: 1, fontSize: 11, color: ink2, lineHeight: 1.5 }}>
              You can be in <b style={{ color: ink }}>up to 10 active teams</b>. Tournaments may restrict multi-team play per their own rules.
            </div>
          </div>
        </div>
      </div>
  ...
  // ── POPULATED STATE ──────────────────────────────────
  const captained = {
    name: 'Lahore Lions', short: 'LL', color: red, role: 'Captain',
    members: 18, jersey: 7, founded: 2019, area: 'Model Town',
    record: { p: 47, w: 31, l: 14, t: 2 },
    nextFixture: { vs: 'Eagles', when: 'Today · 18:30', tag: 'Spring QF', urgent: true },
    duties: 3, // captain duties pending
  };
  const otherTeams = [
    {
      name: 'Old Boys', short: 'OB', color: ink, role: 'Player',
      members: 22, jersey: 11, area: 'University Town',
      record: { p: 12, w: 7, l: 5 },
      nextFixture: { vs: 'DHA', when: 'Sun 19 · 16:00', tag: 'Friendly' },
    },
    {
      name: 'Office Tigers', short: 'OT', color: green, role: 'Manager',
      members: 11, jersey: null, area: 'I.I. Chundrigar',
      record: { p: 6, w: 4, l: 2 },
      nextFixture: null,
    },
  ];
  const archived = [
    { name: 'Aitchison U-19', short: 'AIT', color: 'oklch(0.55 0.15 250)', role: 'Player', record: { p: 18, w: 12, l: 6 }, ended: 'Left Jun 2023' },
  ];

  const RoleBadge = ({ role }) => (
    <PvPill kind={role === 'Captain' ? 'red' : role === 'Manager' ? 'ink' : 'default'}>
      {role === 'Captain' && '✦ '}{role}
    </PvPill>
  );
  ...
  return (
    <div>
      <PvHeader
        title="My teams"
        sub={`1 captained · 1 player · 1 managed · 1 archived`}
        onBack={back}
      />

      {/* ── Primary / captained team — same row style as below ── */}
      <PvSectionH accent={red}>Your side</PvSectionH>
      <div style={{ padding: '0 18px' }}>
        <PvCard accent={captained.color}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{
              width: 40, height: 40, borderRadius: 9,
              background: captained.color, color: paper,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: 'Inter Tight', fontWeight: 700, fontSize: 14,
              flexShrink: 0,
            }}>{captained.short}</div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                <RoleBadge role={captained.role} />
                <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, letterSpacing: '0.06em' }}>
                  #{captained.jersey}
                </span>
              </div>
              <div style={{ fontWeight: 700, fontSize: 14, fontFamily: 'Inter Tight', letterSpacing: '-0.01em' }}>{captained.name}</div>
              <div style={{ display: 'flex', gap: 10, marginTop: 4, fontFamily: 'JetBrains Mono', fontSize: 10, color: muted, letterSpacing: '0.04em' }}>
                <span>{captained.record.p}P · {captained.record.w}W · {captained.record.l}L</span>
                <span>·</span>
                <span style={{ color: captained.nextFixture.urgent ? red : muted, fontWeight: captained.nextFixture.urgent ? 700 : 400 }}>
                  NEXT {captained.nextFixture.when.split(' · ')[0].toUpperCase()}
                </span>
              </div>
            </div>
      ...
      {/* ── Other active teams ───────────────────────────── */}
      <PvSectionH count={otherTeams.length}>Also playing for</PvSectionH>
      <div style={{ padding: '0 18px', display: 'grid', gap: 8 }}>
        {otherTeams.map((t, i) => (
          <PvCard key={i}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                width: 40, height: 40, borderRadius: 9,
                background: t.color, color: paper,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'Inter Tight', fontWeight: 700, fontSize: 14,
                flexShrink: 0,
              }}>{t.short}</div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 14, fontFamily: 'Inter Tight', letterSpacing: '-0.01em' }}>{t.name}</div>
                <div style={{ display: 'flex', gap: 10, marginTop: 4, fontFamily: 'JetBrains Mono', fontSize: 10, color: muted, letterSpacing: '0.04em' }}>
                  <span>{t.record.p}P · {t.record.w}W · {t.record.l}L</span>
                  {t.nextFixture && (
                    <>
                      <span>·</span>
                      <span>NEXT {t.nextFixture.when.split(' · ')[0]}</span>
                    </>
                  )}
                </div>
              </div>
      ...
      {/* ── Archived / inactive ──────────────────────────── */}
      <PvSectionH count={archived.length}>Archived</PvSectionH>
      <div style={{ padding: '0 18px', display: 'grid', gap: 8 }}>
        {archived.map((t, i) => (
          <button key={i} style={{
            width: '100%', textAlign: 'left', fontFamily: 'inherit', cursor: 'pointer',
            background: 'transparent', border: '1px dashed ' + hair,
            ...
            <div style={{
              ...
              flexShrink: 0, filter: 'saturate(0.6)',
            }}>{t.short}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 600, fontSize: 13, color: ink2 }}>{t.name}</div>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: muted, marginTop: 3, letterSpacing: '0.04em' }}>
                {t.ended} · {t.record.p}P · {t.record.w}W · {t.record.l}L
              </div>
            </div>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={muted} strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
          </button>
        ))}
      </div>

      {/* ── Create / join more ───────────────────────────── */}
      <PvSectionH>Want another team?</PvSectionH>
      <div style={{ padding: '0 18px 28px', display: 'grid', gap: 8 }}>
        <button style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: ink, fontFamily: 'Inter Tight', letterSpacing: '-0.01em' }}>Start a new team</div>
            <div style={{ fontSize: 11, color: muted, marginTop: 2, lineHeight: 1.4 }}>5 steps · name, crest, home, roster, invite</div>
          </div>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={muted} strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
        </button>
        <button style={{
          ...
          border: '1px dashed ' + hair, borderRadius: 10,
          cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit',
        }}>
          <div style={{
            width: 36, height: 36, borderRadius: 9,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 600, fontSize: 13, color: ink }}>Find or join a team</div>
            <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>24 within 50 km · or enter an invite code</div>
          </div>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={muted} strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
        </button>
      </div>
...
function CkPavilion({ face = 'captain', initialView = 'home' }) {
  ...
  const screens = {
    ...
    'my-matches':               () => <PvMyMatches back={back} face={face} />,
    'my-teams':                 () => <PvMyTeams back={back} empty={face === 'new'} />,
