...
// 5 steps: Basics → Identity → Home → Crest → Review → Done
// Squad is built AFTER creation, from the team page — not in this flow.
...
function CkTeamCreate({ initialStep = 1 } = {}) {
  ...
  const STEPS = ['Basics', 'Identity', 'Home', 'Crest', 'Review'];
  const isLast = step === STEPS.length;

  // ── Done / success terminal state ────────────────
  // After Create team, land directly on the team page with a brief banner.
  // No separate "Team created!" screen — the team page IS the celebration.
  if (done) return <CkTeamManage justCreated team={{
    id: 'NEW', name: team.name, primary: team.primary,
    monogram: team.monogram, type: team.type, city: team.city, area: team.area,
  }} />;
  ...
  return (
    <div className="ck-screen" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        {step === 5 && <TCStepReview team={team} jumpTo={setStep} />}
        <div style={{ height: 30 }} />
      </div>
      ...
      <div style={{ flexShrink: 0, padding: '10px 16px 18px', borderTop: '1px solid var(--hairline)', background: 'var(--paper)', display: 'flex', gap: 8 }}>
        ...
        <button onClick={() => isLast ? setDone(true) : setStep(step + 1)} style={{
          ...
          {isLast ? 'Create team' : (step === 4 ? 'Review' : 'Continue')}
          {!isLast && (
...
function TCStepCrest({ team, setT }) {
  ...
  const styles = ['Monogram', 'Initials', 'Shield'];
  ...
  const handleFile = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      // Mock validation — in real app, show a toast
      alert('Max 2MB. Crop or compress and try again.');
      return;
    }
    const reader = new FileReader();
    reader.onload = (ev) => setT({ logoUrl: ev.target.result, logoName: file.name, logoSize: file.size, crestStyle: 'Upload' });
    reader.readAsDataURL(file);
  };

  const removeLogo = () => setT({ logoUrl: null, logoName: null, logoSize: null, crestStyle: 'Monogram' });
  ...
  return (
    <div style={{ padding: '4px 18px 0' }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--muted)', margin: '0 0 18px', lineHeight: 1.4 }}>
        Upload your club's logo if you have one, or pick a generated style.
      </p>

      {/* ── Big preview ───────────────────────────────── */}
      <div style={{
        margin: '0 auto 22px', width: 132, height: 132, borderRadius: 28,
        background: hasLogo ? 'var(--paper-2)' : team.primary,
        color: textOn(team.primary),
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontFamily: 'Inter Tight', fontWeight: 800, fontSize: 56, letterSpacing: '-0.04em',
        position: 'relative', overflow: 'hidden',
        border: hasLogo ? '1px solid var(--hairline)' : 'none',
      }}>
        {hasLogo ? (
          <img
            src={team.logoUrl}
          ...
          <>
            ...
            <span style={{ position: 'relative', zIndex: 1 }}>
              {team.crestStyle === 'Initials' ? team.monogram[0] : team.monogram}
            </span>
          </>
        )}
      </div>
      ...
      {/* ── Upload path ───────────────────────────────── */}
      <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 700, letterSpacing: '0.10em', color: 'var(--muted)', textTransform: 'uppercase', margin: '0 4px 8px' }}>
        Your logo
      </div>
      ...
      {hasLogo ? (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {team.logoName || 'team-logo.png'}
              </span>
              <span style={{ fontFamily: 'JetBrains Mono', fontSize: 9, color: 'var(--green)', fontWeight: 700, letterSpacing: '0.08em', padding: '2px 5px', background: 'var(--green-soft)', borderRadius: 4 }}>
                UPLOADED
              </span>
            </div>
            <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--muted)', marginTop: 2, letterSpacing: '0.04em' }}>
              {team.logoSize ? `${Math.round(team.logoSize / 1024)} KB` : '—'} · auto-cropped square
            </div>
          </div>
          <button
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid var(--hairline)', background: 'var(--paper)',
              color: 'var(--muted)', fontSize: 11, fontFamily: 'inherit', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
          >
        ...
        <button
          ...
          style={{
            ...
            padding: '14px 16px',
            background: 'var(--paper-2)', border: '1.5px dashed var(--soft)',
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--ink)', fontFamily: 'Inter Tight', letterSpacing: '-0.01em' }}>
              Upload a logo
            </div>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 3, lineHeight: 1.4 }}>
              PNG, JPG or SVG · max 2 MB · transparent background recommended
            </div>
          </div>
        </button>
      )}
      ...
      <div style={{
        ...
        letterSpacing: '0.10em', color: 'var(--muted)', textTransform: 'uppercase',
      }}>
        <span style={{ flex: 1, height: 1, background: 'var(--hairline)' }} />
        <span>{hasLogo ? 'Or generate one' : 'Or use a generated crest'}</span>
        <span style={{ flex: 1, height: 1, background: 'var(--hairline)' }} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
        {styles.map(s => (
          <button key={s} onClick={() => setT({ crestStyle: s, logoUrl: null, logoName: null, logoSize: null })} style={{
            padding: '14px 10px', borderRadius: 12, cursor: 'pointer', textAlign: 'center',
            border: (!hasLogo && team.crestStyle === s) ? '1.5px solid var(--ink)' : '1px solid var(--hairline)',
            background: (!hasLogo && team.crestStyle === s) ? 'var(--paper)' : 'var(--surface)',
            fontFamily: 'Inter', fontSize: 12, fontWeight: 600, color: 'var(--ink)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
            opacity: hasLogo ? 0.5 : 1,
          }}>
      ...
      <div style={{ padding: 12, background: 'var(--paper-2)', borderRadius: 12, fontSize: 11, color: 'var(--ink-2)', lineHeight: 1.4 }}>
        {hasLogo
          ? <>Your uploaded logo will appear on scorecards, team pages and the bracket. We'll auto-tint it to your team colors where contrast is needed.</>
          : <>Crest auto-syncs with your team colors. You can replace it with an upload anytime.</>
        }
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: '4px 18px 0' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--hairline)', borderRadius: 14, overflow: 'hidden', marginBottom: 16 }}>
        ...
        <SummaryRow label="CREST"    step={4} value={team.logoUrl ? `Logo uploaded · ${team.logoName || 'logo'}` : `${team.crestStyle} style · auto-synced colors`} />
      </div>
      ...
      <div style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.5, padding: '0 4px 4px' }}>
        By creating this team you agree to Circk's community guidelines. You'll be able to add players from the team page next.
      </div>
    </div>
  );
}
    ...
    <div className="ck-screen" style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--paper)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '22px 18px 18px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--hairline)', borderRadius: 14, padding: '4px 14px', marginBottom: 22 }}>
          ...
          <ReceiptRow icon="check" label={team.logoUrl ? 'Logo uploaded' : 'Crest set'} detail={team.logoUrl ? (team.logoName || 'team-logo.png') : `${team.crestStyle} style`} />
          <ReceiptRow icon="dot" label="Squad pending" detail="Add players from the team page" />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 18 }}>
          <NextRow primary label="Add players" sub={`Up to ${team.maxSize} · search, SMS, or unclaimed`} icon={
          ...
          <NextRow label="Open team page" sub="See your public profile" icon={
