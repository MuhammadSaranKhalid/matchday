      ...
      <div style={{ flexShrink: 0, padding: '10px 16px 18px', borderTop: '1px solid var(--hairline)', background: 'var(--paper)', display: 'flex', gap: 8 }}>
        <button
          onClick={() => window.openCkComposer && window.openCkComposer({
            author: { kind: 'tournament', name: "Spring Cup '26", initials: 'SC', color: 'oklch(0.45 0.12 280)' },
          })}
          style={{ flex: 1, padding: '12px 0', borderRadius: 12, border: 'none', background: 'oklch(0.45 0.12 280)', color: 'white', fontFamily: 'Inter', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, boxShadow: '0 4px 10px rgba(80,40,160,0.25)' }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><path d="M12 5v14M5 12h14"/></svg>
          Post update
        </button>
        <button style={{ padding: '12px 16px', borderRadius: 12, border: '1px solid var(--hairline)', background: 'var(--paper)', color: 'var(--ink)', fontFamily: 'Inter', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
          Share invite
        </button>
      </div>
