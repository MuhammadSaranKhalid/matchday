...
const POSTS = [
  {
    id: 'live1', kind: 'liveHero', filter: ['match', 'teams', 'tour'], bucket: 'now',
    home: { name: 'Lahore Lions', short: 'LAH', color: 'oklch(0.62 0.19 28)', baseRuns: 132, baseWickets: 4, baseBalls: 87 },
    away: { name: 'City Eagles', short: 'CTY', color: 'oklch(0.42 0.10 260)', runs: 178, wickets: 10, balls: 120 },
    target: 179, batting: 'home', tag: "Spring Cup '26 · Quarterfinal", venue: 'Model Town · Lahore',
    striker: { name: 'A. Khan', runs: 52, balls: 41 }, nonStriker: { name: 'S. Ali', runs: 28, balls: 22 },
    bowler: { name: 'R. Patel', figures: '2/24 (3.5)' },
  },
  {
...
const BUCKET_LABELS = {
  now: { label: 'NOW', side: 'Live · 1 match' },
...
function Feed() {
  ...
  const onLink = (kind) => {
    if (window.__ckNav) {
      if (kind === 'match') window.__ckNav.push('match');
      else if (kind === 'tournament') window.__ckNav.push('tournament');
      else if (kind === 'team') window.__ckNav.push('team');
      return;
    }
  ...
  const renderPost = (post) => {
    if (post.kind === 'liveHero') return <LiveHero key={post.id} post={post} onLink={onLink} />;
    ...
    <div className="ck-screen" style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      {tab !== 'home' ? (
        ...
        <>
          ...
          {!window.__ckNav && <Fab onClick={() => setComposing(true)} />}
        </>
      )}
