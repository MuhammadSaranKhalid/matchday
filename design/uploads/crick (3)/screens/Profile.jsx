...
function FollowRow({ following, setFollowing, isMe }) {
  // When viewing your own profile, the primary CTA is "Post".
  // Follow / Message are for OTHER players.
  if (isMe) {
    return (
      <div style={{ display: 'flex', gap: 8, padding: '0 20px 14px' }}>
        <button
          onClick={() => window.openCkComposer && window.openCkComposer({ author: window.CK_COMPOSER_AUTHORS.personal })}
          style={{
            flex: 1, padding: '12px 0', borderRadius: 10, border: 'none',
            background: 'var(--red)', color: 'white',
            fontWeight: 700, fontSize: 14, fontFamily: 'inherit', cursor: 'pointer',
            boxShadow: '0 4px 12px rgba(190,60,40,0.28)',
        ...
        <button style={{
          ...
          color: 'var(--ink)', fontWeight: 600, fontSize: 13, fontFamily: 'inherit', cursor: 'pointer',
        }}>Edit profile</button>
      </div>
    );
  }
...
function ProfileA({ isMe = true }) {
  ...
  return (
    <div className="ck-screen" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <ProfileHeader />
      <FollowRow following={following} setFollowing={setFollowing} isMe={isMe} />
