...
**R1 — Drawer edge-drag vs. page swipe.** Tabs are laid out in a `PageView`
(`swipeable_branch_view.dart`) for swipe navigation. Flutter's `DrawerController` claims a
~20dp left-edge gesture, and both want the same pixels on tab 0. Mitigation: set
`drawerEdgeDragWidth` explicitly and, if needed, stop the `PageView` consuming horizontal
drags originating in that strip. **This needs verification on a device, not reasoning.**
...
- **The drawer is presentation-only**, living in `shell` — a presentation-only feature per
...
- **Sign out** goes `panel → authControllerProvider → authRepository`, matching the
