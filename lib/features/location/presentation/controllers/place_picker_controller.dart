...
class PlacePicker extends _$PlacePicker {
  ...
  void setQuery(String q) {
    _ticket++; // Invalidate in-flight work immediately, not after debounce.
    state = state.copyWith(query: q, picked: null, error: null,
      resolving: false, suggestions: const [], searching: q.trim().length >= _minQueryLen);
    _debounce?.cancel();
  ...
  Future<void> select(PlaceSuggestion suggestion) async {
    ...
    if (!ref.mounted || ticket != _ticket) return;
  ...
  Future<void> _autocomplete() async {
    ...
    state = res.fold(
      (failure) => state.copyWith(searching: false, suggestions: const [], error: failure),
