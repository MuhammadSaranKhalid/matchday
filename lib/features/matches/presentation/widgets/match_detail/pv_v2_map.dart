...
PvCrest crestFromTeam(Team t) => PvCrest(
  short:
      (t.logoMonogram != null && t.logoMonogram!.isNotEmpty)
          ? t.logoMonogram!
          : teamMonogram(t.name),
  name: t.name,
...
List<PvMatch> pvMatchesFromView(
  MyMatchesView v, {
  required PvCrest meFallback,
}) {
  final out = <PvMatch>[];
  ...
  for (final c in v.confirmed) {
    ...
    final roleLabel = switch (c.roleKind) {
      MatchRoleKind.captain => 'captain',
      MatchRoleKind.scoring => 'official',
      MatchRoleKind.xi => 'player',
      MatchRoleKind.optional => 'player',
      MatchRoleKind.spectator => 'viewer',
    };
    out.add(
      PvMatch(
        id: c.id,
        phase:
            c.live
                ? PvPhase.live
                : (c.tossReady ? PvPhase.startsSoon : PvPhase.scheduled),
        // Respect the existing youIsHome field.
        //
        // Never blindly call Team A "me".
        me: c.youIsHome ? home : away,
        them: c.youIsHome ? away : home,
        role: roleLabel,
        meSubtitle: 'you · $roleLabel',
        themSubtitle: 'opponent',
        canStartMatch: c.tossReady,
        when: c.when,
        venue: c.venue,
        sub: c.tag,
...
String _teamTypeLabel(TeamType type) =>
    type.wire.isEmpty
        ? ''
        : '${type.wire[0].toUpperCase()}${type.wire.substring(1)}';
