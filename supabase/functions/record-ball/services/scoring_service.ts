  ...
  async recordDelivery(
    ...
    return await sql.begin(
      async (tx) => {
        ...
        const inningsId =
          lockedInnings.inningsId;

        const match =
          await matchRepository.getMatch(
            tx,
            input.matchId,
          );
        ...
        let transition:
          RecordDeliveryResult["transition"] =
            { kind: "none" };
...
function toInningsLines(
  ...
  const teamA =
    (match?.team_a_id ?? null) as string | null;

  const teamB =
    (match?.team_b_id ?? null) as string | null;

  const tossSide =
    (match?.toss_won_by ?? null) as "team_a" | "team_b" | null;

  const decision =
    (match?.toss_decision ?? null) as string | null;
