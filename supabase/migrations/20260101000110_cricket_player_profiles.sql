-- Migration file: 20260101000110_cricket_player_profiles.sql
...
create trigger cricket_player_profiles_set_updated_at
  before update on public.cricket_player_profiles for each row
...
alter table public.cricket_player_profiles
  add constraint cricket_player_profiles_player_sport_fkey foreign key (user_id, sport_id) references public.player_sports(user_id, sport_id) on update restrict on delete cascade;

comment on table public.cricket_player_profiles is 'Cricket-specific player attributes. A row requires the corresponding '
  '(user_id, cricket) identity in player_sports.';

comment on column public.cricket_player_profiles.sport_id is 'Always cricket. Present to enforce the composite FK to player_sports.';
...
create policy "cricket_player_profiles_read_public" on public.cricket_player_profiles
  for select to anon, authenticated
  using (true);

create policy "cricket_player_profiles_insert_self" on public.cricket_player_profiles
...
create policy "cricket_player_profiles_update_self" on public.cricket_player_profiles
...
create policy "cricket_player_profiles_delete_self" on public.cricket_player_profiles
