-- Migration file: 20260101000206_role_permissions.sql
...
create table public.role_permissions(
  ...
  foreign key (permission_key, scope) references public.permission_scopes(permission_key, scope) on delete cascade,
...
create index idx_role_permissions_lookup on public.role_permissions(scope, role_key, permission_key);
...
begin
  ...
  if v_rank < v_min then
    raise exception 'Permission % requires rank >= %; role % has rank %', new.permission_key, v_min, new.role_key, v_rank
      using errcode = '42501';
  end if;
  return new;
end;
...
create trigger role_permissions_min_rank
  before insert or update on public.role_permissions for each row
  execute function public.guard_permission_min_rank();
...
where
  ...
  and (
    ...
    or (r.key = 'captain'
      and p.permission_key in ('match.lineup.set', 'match.score', 'match.official.assign', 'team.contact.view'))
    -- player: nothing.
);
