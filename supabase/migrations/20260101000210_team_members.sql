-- =============================================================================
-- Migration: 20260101000210_team_members.sql
-- =============================================================================
...
create table public.team_members (
  ...
  constraint left_at_for_inactive
    check (
...
create index team_members_team
  on public.team_members (team_id);

create index team_members_user
  on public.team_members (user_id)
  where user_id is not null;

create index team_members_unclaimed
  on public.team_members (unclaimed_id)
  where unclaimed_id is not null;

create index idx_team_members_added_by
...
begin
  ...
  if
    v_team_sport is not null
    and v_player_sport is not null
    and v_team_sport is distinct from v_player_sport
  then
    raise exception 'Unclaimed player sport (%) does not match team sport (%)',
      v_player_sport,
      v_team_sport
      using errcode = '23514';
  end if;
...
create trigger team_members_enforce_unclaimed_sport
  before insert or update of team_id, unclaimed_id on public.team_members
  for each row
...
begin
  ...
  if v_sport_id is null then
    raise exception 'Team sport could not be resolved' using errcode = 'P0002';
  end if;
...
create trigger team_members_activate_player_sport
  after insert or update of user_id, team_id, in_squad, status on public.team_members
  for each row
