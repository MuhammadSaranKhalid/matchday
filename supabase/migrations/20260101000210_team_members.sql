-- Migration file: 20260101000210_team_members.sql
...
create table public.team_members(
  ...
  constraint left_at_for_inactive check ((status = 'active' and left_at is null) or (status in ('inactive', 'removed') and left_at is not null)),
...
create unique index team_members_unique_active_user on public.team_members(team_id, user_id)
where
  status = 'active' and user_id is not null;

create unique index team_members_unique_active_unclaimed on public.team_members(team_id, unclaimed_id)
where
  status = 'active' and unclaimed_id is not null;

create unique index team_members_unique_jersey on public.team_members(team_id, jersey_number)
where
  status = 'active' and jersey_number is not null;

create unique index team_members_unique_primary_per_user on public.team_members(user_id)
where
  is_primary = true and status = 'active' and user_id is not null;
...
create trigger team_members_set_updated_at
  before update on public.team_members for each row
...
begin
  ...
  if v_team_sport is not null and v_player_sport is not null and v_team_sport is distinct from v_player_sport then
    raise exception 'Unclaimed player sport (%) does not match team sport (%)', v_player_sport, v_team_sport
      using errcode = '23514';
  end if;
  return new;
end;
...
revoke all on function public.enforce_unclaimed_team_sport() from public, anon, authenticated;

-- Section: Triggers (continued)

create trigger team_members_enforce_unclaimed_sport
  before insert or update of team_id,
  unclaimed_id on public.team_members for each row
...
revoke all on function public.activate_player_sport_from_team_membership() from public, anon, authenticated;
...
create trigger team_members_activate_player_sport
  ...
  team_id,
  in_squad,
  status on public.team_members for each row
