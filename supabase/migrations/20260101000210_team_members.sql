-- =============================================================================
-- 0210 · team_members + team_member_roles + the authorization engine
-- =============================================================================
...
--   `{owner, manager, player}` max 1 (0205). `captain` is in no set, so
...
create table public.team_member_roles (
  ...
  foreign key (scope, role_key, is_singleton)
    references public.roles (scope, key, is_singleton) on update cascade,
  constraint team_member_roles_membership_fk
    foreign key (membership_id, team_id)
      references public.team_members (membership_id, team_id) on delete cascade
);
...
create unique index team_member_roles_singleton_per_team
  on public.team_member_roles (team_id, scope, role_key)
  where is_singleton;

create index team_member_roles_membership on public.team_member_roles (membership_id);
create index team_member_roles_team_role  on public.team_member_roles (team_id, scope, role_key);
create index team_member_roles_granted_by on public.team_member_roles (granted_by);
...
begin
  ...
  select s.name
    into v_set_name
    from public.role_exclusion_sets s
    join public.role_exclusion_members m
      on m.set_id = s.set_id
     and m.scope = new.scope
     and m.role_key = new.role_key
   where (
     select count(*)
  ...
  if v_set_name is not null then
    raise exception
      'Role "%" breaches exclusion set "%" for this member',
      new.role_key, v_set_name
      using errcode = '23514';
  end if;
  return null;
end;
...
create constraint trigger team_member_roles_exclusion
  ...
  for each row execute function public.guard_role_exclusion();
...
begin
  select user_id into v_user_id
    from public.team_members where membership_id = new.membership_id;
  if v_user_id is not null then
    return new;
  end if;
  select allows_unclaimed into v_allows
    from public.roles where scope = new.scope and key = new.role_key;
  if not coalesce(v_allows, false) then
    raise exception
      'Role "%" requires a MatchDay account; this member is an unclaimed placeholder',
      new.role_key
      using errcode = '42501';
  end if;
  return new;
...
create trigger team_member_roles_needs_account
  ...
  for each row execute function public.guard_role_needs_account();
...
begin
  if not exists (
    ...
    return null;   -- membership gone or inactive: nothing to check
  end if;
  if not exists (
    select 1 from public.team_member_roles where membership_id = v_membership
  ) then
    raise exception 'A team member must hold at least one role'
      using errcode = '23514';
  end if;
  return null;
end;
...
create constraint trigger team_member_roles_at_least_one
  ...
  for each row execute function public.guard_member_has_role();
...
-- =============================================================================
-- Declared here rather than 0205 because it reads team_members and
...
-- =============================================================================
-- Policies deferred from 0200 (teams) and 0205 (authz)
-- =============================================================================
...
-- checked at CREATE time — and reads teams (0200) and team_members (this file).
