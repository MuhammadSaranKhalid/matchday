-- Migration file: 20260101000211_team_member_roles.sql
...
create table public.team_member_roles(
  ...
  foreign key (scope, role_key, is_singleton) references public.roles(scope, key, is_singleton) on update cascade,
  constraint team_member_roles_membership_fk foreign key (membership_id, team_id) references public.team_members(membership_id, team_id) on delete cascade
);
...
create unique index team_member_roles_singleton_per_team on public.team_member_roles(team_id, scope, role_key)
where
  is_singleton;
...
begin
  ...
  from
    ...
    join public.role_exclusion_members m on m.set_id = s.set_id
  ...
  where (
    ...
    from
      ...
      join public.role_exclusion_members m2 on m2.set_id = s.set_id
        and m2.scope = tmr.scope
        and m2.role_key = tmr.role_key
    where
      tmr.membership_id = new.membership_id) > s.max_roles
limit 1;
  if v_set_name is not null then
    raise exception 'Role "%" breaches exclusion set "%" for this member', new.role_key, v_set_name
      using errcode = '23514';
  end if;
  return null;
end;
...
create constraint trigger team_member_roles_exclusion
  after insert or update on public.team_member_roles for each row
...
begin
  ...
  if not coalesce(v_allows, false) then
    raise exception 'Role "%" requires a MatchDay account; this member is an unclaimed placeholder', new.role_key
      using errcode = '42501';
  end if;
  return new;
end;
...
create trigger team_member_roles_needs_account
  before insert or update on public.team_member_roles for each row
...
begin
  ...
  if not exists (
    ...
    where
      membership_id = v_membership) then
...
create constraint trigger team_member_roles_at_least_one
  after insert or update or delete on public.team_member_roles deferrable initially deferred for each row
