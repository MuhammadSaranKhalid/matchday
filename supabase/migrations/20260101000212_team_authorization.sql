...
create or replace function public.team_can(
  p_team_id uuid,
  p_permission text
)
returns boolean
...
begin
  perform
    public._attach_role(new.membership_id, coalesce(v_role, 'player'), new.added_by);
  -- Reset so a second insert in the same transaction does not inherit it.
  perform
    set_config('matchday.initial_role', '', true);
  return null;
end;
$$;
...
begin
  ...
  if v_target is distinct from v_uid and v_mine <= public._member_rank(p_membership_id) then
  ...
  if exists (
    ...
    and exists (
      select
        1
      from
        public.role_exclusion_members a
        join public.role_exclusion_members b on b.set_id = a.set_id
      where
        a.scope = 'team'
        and a.role_key = 'owner'
        and b.scope = 'team'
        and b.role_key = p_role_key) then
  ...
  where tmr.membership_id = p_membership_id
    ...
    and exists (
      ...
      where
        a.scope = tmr.scope
        and a.role_key = tmr.role_key
        and b.scope = 'team'
        and b.role_key = p_role_key);
  perform
    public._attach_role(p_membership_id, p_role_key, v_uid);
end;
$$;
...
begin
  ...
  perform
    public._attach_role(v_theirs, 'owner', v_uid);
end;
$$;
...
begin
  ...
  if v_is_owner then
    raise exception 'Transfer ownership before leaving the team'
      using errcode = '42501';
  end if;
