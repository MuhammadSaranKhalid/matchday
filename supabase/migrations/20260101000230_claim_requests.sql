...
create table public.claim_requests (
  ...
  unclaimed_id   uuid not null
                     references public.unclaimed_players(unclaimed_id) on delete cascade,
  ...
  constraint claim_request_decision_consistency check (
...
create unique index claim_requests_one_pending
  on public.claim_requests (unclaimed_id, requester_id)
...
-- -----------------------------------------------------------------------------
create or replace function public.approve_claim_request(p_request_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  select cr.unclaimed_id, cr.requester_id, up.added_by
