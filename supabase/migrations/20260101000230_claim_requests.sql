-- Migration file: 20260101000230_claim_requests.sql
...
create table public.claim_requests(
  ...
  unclaimed_id uuid not null references public.unclaimed_players(unclaimed_id) on delete cascade,
  ...
  constraint claim_request_decision_consistency check ((status = 'pending' and decided_by is null and decided_at is null) or (status in ('approved', 'rejected') and decided_at is not null) or (status = 'cancelled'))
);

-- Section: Indexes

create unique index claim_requests_one_pending on public.claim_requests(unclaimed_id, requester_id)
where
  status = 'pending';

create index claim_requests_unclaimed on public.claim_requests(unclaimed_id);

create index claim_requests_requester on public.claim_requests(requester_id);

-- Section: Triggers

create trigger claim_requests_set_updated_at
  before update on public.claim_requests for each row
...
begin
  ...
  set
    claimed_by_user_id = v_requester_id
  where
...
create policy "claim_requests_read_self_or_owner" on public.claim_requests
...
create policy "claim_requests_insert_self" on public.claim_requests
  for insert to authenticated
  with check ((
    select
      auth.uid()) = requester_id);

create policy "claim_requests_update_self_or_owner" on public.claim_requests
...
create index if not exists idx_claim_requests_decided_by on public.claim_requests(decided_by);
