-- Migration file: 20260612000000_team_join_requests.sql
...
create table public.team_join_requests(
  ...
  constraint join_request_decision_consistency check ((status = 'pending' and decided_by is null and decided_at is null) or (status in ('approved', 'rejected') and decided_at is not null) or (status = 'cancelled'))
);

-- Section: Indexes

create unique index team_join_requests_one_pending on public.team_join_requests(team_id, player_id)
where
  status = 'pending';
...
create trigger team_join_requests_set_updated_at
  before update on public.team_join_requests for each row
...
begin
  ...
  if not public.is_team_manager(v_team_id) then
    raise exception 'Only team managers can approve join requests'
      using errcode = '42501';
  end if;
  perform
  ...
  if v_role = 'captain' then
    perform
      public._attach_role(v_membership_id, 'player', v_uid);
  end if;
  update
    public.team_join_requests
  set
...
begin
  ...
  if not public.is_team_manager(v_team_id) then
    raise exception 'Only team managers can decline join requests'
      using errcode = '42501';
  end if;
  update
    public.team_join_requests
...
create index if not exists idx_team_join_requests_decided_by on public.team_join_requests(decided_by);
