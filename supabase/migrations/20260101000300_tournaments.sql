...
create table public.tournaments (
  ...
  tournament_name        text not null
                          check (length(tournament_name) between 3 and 100),
  ...
  constraint min_le_max_teams check (
  ...
  constraint deadline_before_start check (
    registration_deadline is null or start_date is null
...
values
  ('tournament-banners', 'tournament-banners', true, 10 * 1024 * 1024,
   array['image/jpeg', 'image/png', 'image/webp']),
  ('tournament-logos',   'tournament-logos',   true,  5 * 1024 * 1024,
   array['image/jpeg', 'image/png', 'image/webp'])
