-- =============================================================================
-- Migration: 20260101000300_tournaments.sql
-- =============================================================================
...
create table public.tournaments (
  ...
  prize_details         text
    check (prize_details is null or length(prize_details) <= 500),
...
create index tournaments_creator
  on public.tournaments (created_by);

create index tournaments_organizers_gin
  on public.tournaments using gin (organizers);

create index tournaments_status
  on public.tournaments (status);

create index tournaments_start_date
  on public.tournaments (start_date);

create index tournaments_sport_id
...
create trigger tournaments_sport_immutable
  before update of sport_id on public.tournaments
  for each row
  ...
  select
    exists (
      ...
      where
        ...
        and (t.created_by = auth.uid() or auth.uid() = any (t.organizers))
    );
$$;
...
create policy "tournament_banners_read_public"
  on storage.objects
  for select
  using (bucket_id = 'tournament-banners');

create policy "tournament_banners_write_organizer"
  on storage.objects
  for all
  to authenticated
  using (
...
create policy "tournament_logos_read_public"
  on storage.objects
  for select
  using (bucket_id = 'tournament-logos');

create policy "tournament_logos_write_organizer"
  on storage.objects
  for all
  to authenticated
  using (
