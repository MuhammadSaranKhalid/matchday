-- Migration file: 20260101000310_tournament_teams.sql
...
create table public.tournament_teams(
  ...
  payment_channel     text check (payment_channel is null or payment_channel in ('cash', 'jazzcash', 'easypaisa', 'bank_transfer', 'other')),
  ...
  constraint registration_decision_consistency check ((status in ('pending', 'withdrawn')) or (status in ('approved', 'rejected') and decided_at is not null))
);
...
begin
  ...
  if v_tournament_sport is not null and v_team_sport is not null and v_tournament_sport is distinct from v_team_sport then
    raise exception 'Team sport (%) does not match tournament sport (%)', v_team_sport, v_tournament_sport
      using errcode = '23514';
  end if;
  return new;
end;
...
revoke all on function public.enforce_tournament_team_sport() from public, anon, authenticated;

-- Section: Triggers

create trigger tournament_teams_enforce_sport
  before insert or update of tournament_id,
  team_id on public.tournament_teams for each row
...
create index if not exists tournament_teams_squad_gin on public.tournament_teams using gin(squad);

-- Section: Triggers (continued)

create trigger tournament_teams_set_updated_at
  before update on public.tournament_teams for each row
...
begin
  ...
  if not public.is_tournament_organizer(v_tournament_id) then
    raise exception 'Only tournament organizers can approve'
      using errcode = '42501';
  end if;
  update
    public.tournament_teams
...
begin
  ...
  if not public.is_tournament_organizer(v_tournament_id) then
    raise exception 'Only tournament organizers can reject'
      using errcode = '42501';
  end if;
  update
    public.tournament_teams
...
create policy "tournament_teams_update_manager_or_organizer" on public.tournament_teams
...
create index if not exists idx_tournament_teams_decided_by on public.tournament_teams(decided_by);

create index if not exists idx_tournament_teams_payment_recorded_by on public.tournament_teams(payment_recorded_by);

create index if not exists idx_tournament_teams_registered_by on public.tournament_teams(registered_by);
