-- Migration file: 20260101000331_tournament_grounds.sql
...
create table if not exists public.tournament_grounds(
  ...
  ground_id     uuid not null references public.grounds(ground_id) on delete restrict,
...
create index if not exists tournament_grounds_ground on public.tournament_grounds(ground_id);
...
create policy "tournament_grounds_read" on public.tournament_grounds
  ...
  using (public.is_tournament_organizer(tournament_id)
    or exists (
      ...
      where
        t.tournament_id = tournament_grounds.tournament_id
        and t.privacy = 'public'));

drop policy if exists "tournament_grounds_write_organizer" on public.tournament_grounds;

create policy "tournament_grounds_write_organizer" on public.tournament_grounds
