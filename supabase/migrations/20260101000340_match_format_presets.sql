-- Migration file: 20260101000340_match_format_presets.sql
...
create or replace view public.format_presets with ( security_invoker = on
) as
select
  *
from
  public.match_format_presets;
...
create policy "match_format_presets_read_all" on public.match_format_presets
...
create index if not exists match_format_presets_sport_id on public.match_format_presets(sport_id);

comment on column public.match_format_presets.sport_id is 'Sport whose match rules this preset represents.';

-- Section: Triggers

create trigger match_format_presets_sport_immutable
  before update of sport_id on public.match_format_presets for each row
...
insert into public.match_format_presets(id, label, sort_order, config, is_active, is_system, default_scoring_mode)
values
...
create index idx_match_format_presets_active_order on public.match_format_presets(sport_id, sort_order)
where
  is_active;

-- Index the profile FK for account deletion and joins (Supabase advisor 0001).
create index idx_match_format_presets_created_by on public.match_format_presets(created_by);
