-- =============================================================================
-- Migration: 20260101000400_matches.sql
-- =============================================================================
...
create table public.matches (
  ...
  round                  text,
  bracket_round_number   integer check (
    bracket_round_number is null
    or bracket_round_number >= 1
  ),
  bracket_match_number   integer check (
    bracket_match_number is null
    or bracket_match_number >= 1
  ),
  prev_match_a_id        uuid
    references public.matches (match_id)
    on delete set null,
  prev_match_b_id        uuid
    references public.matches (match_id)
    on delete set null,
  group_id               text,
  venue                  text,
  ground_id              uuid
    references public.grounds (ground_id)
    on delete set null,
...
comment on table public.matches is
  'Sport-neutral match shell. Sport-specific rules, toss/start state, result detail and sport-specific participant roles live in sport extensions.';
