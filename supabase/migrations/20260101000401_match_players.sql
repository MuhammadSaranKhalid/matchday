-- =============================================================================
-- Migration: 20260101000401_match_players.sql
-- =============================================================================
...
create table public.match_players (
  ...
  jersey_number   smallint check (
    jersey_number is null
    or (jersey_number between 0 and 99)
  ),
...
comment on table public.match_players is
  'Sport-neutral participant identity/snapshot for a match. Sport-specific per-match state belongs in sport participant extensions.';
...
create index if not exists idx_match_players_unclaimed
  on public.match_players (
    unclaimed_id
  )
...
begin
  ...
  if
    v_match_sport is not null
    and v_player_sport is not null
    and v_match_sport is distinct from v_player_sport
  then
    raise exception 'Unclaimed player sport (%) does not match match sport (%)',
      v_player_sport,
      v_match_sport
      using errcode = '23514';
  end if;
...
create trigger match_players_enforce_unclaimed_sport
  before insert or update of match_id, unclaimed_id on public.match_players
  for each row
...
begin
  ...
  if v_sport_id is null then
    raise exception 'Match sport could not be resolved' using errcode = 'P0002';
  end if;
...
create trigger match_players_activate_player_sport
  after insert or update of user_id, match_id on public.match_players
  for each row
