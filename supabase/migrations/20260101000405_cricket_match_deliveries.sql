...
create table public.cricket_match_deliveries(
  ...
  innings_id        uuid not null references public.cricket_match_innings(innings_id) on delete cascade,
  ...
  constraint chk_delivery_ball_in_over check ((is_legal_delivery and ball_in_over >= 1) or (not is_legal_delivery and ball_in_over = 0)),
  ...
  constraint chk_delivery_type_legality CHECK (case delivery_type
  when 'wide' then
...
create or replace view public.balls with ( security_invoker = on
) as
select
  *
from
...
create policy "cricket_match_deliveries_write_scorer" on public.cricket_match_deliveries
...
create index if not exists idx_deliveries_match on public.cricket_match_deliveries(match_id);

create index if not exists idx_deliveries_striker on public.cricket_match_deliveries(striker_id)
where
  striker_id is not null;

create index if not exists idx_deliveries_bowler on public.cricket_match_deliveries(bowler_id)
where
  bowler_id is not null;
...
create index if not exists idx_cricket_match_deliveries_fielder_id on public.cricket_match_deliveries(fielder_id);

create index if not exists idx_cricket_match_deliveries_non_striker_id on public.cricket_match_deliveries(non_striker_id);

create index if not exists idx_cricket_match_deliveries_recorded_by on public.cricket_match_deliveries(recorded_by);
...
comment on table public.cricket_match_deliveries is 'CRICKET ENGINE TABLE (legacy generic name). Planned rename: '
