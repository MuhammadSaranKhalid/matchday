-- =============================================================================
-- Migration: 20260101000406_cricket_match_innings_state.sql
-- =============================================================================
...
create table public.cricket_match_innings_state (
  ...
  total_extras     integer not null generated always as (
    total_wides + total_no_balls + total_byes + total_leg_byes + total_penalties
  ) stored,
  ...
  constraint chk_state_distinct_batters
    check (
      striker_id is null
  ...
  constraint cricket_match_innings_state_parent_fkey
    foreign key (
      innings_id,
      match_id,
      innings_number
    )
    references public.cricket_match_innings (innings_id, match_id, innings_number)
    on delete cascade
);
...
create policy "cricket_match_innings_state_write_scorer"
  on public.cricket_match_innings_state
  for all
  to authenticated
  using (true);
...
create index if not exists idx_innings_state_match
  on public.cricket_match_innings_state (
    match_id
  );

create index if not exists idx_cricket_match_innings_state_bowler_id
  on public.cricket_match_innings_state (
    bowler_id
  );

create index if not exists idx_cricket_match_innings_state_non_striker_id
  on public.cricket_match_innings_state (
    non_striker_id
  );

create index if not exists idx_cricket_match_innings_state_striker_id
  on public.cricket_match_innings_state (
    striker_id
  );
