...
create table public.cricket_match_wickets(
  ...
  delivery_id           uuid not null unique references public.cricket_match_deliveries(delivery_id) on delete cascade,
...
create policy "cricket_match_wickets_write_scorer" on public.cricket_match_wickets
  for all to authenticated
  using (true);

-- Section: Indexes

create index if not exists idx_wickets_innings on public.cricket_match_wickets(innings_id);

create index if not exists idx_cricket_match_wickets_assisted_fielder_id on public.cricket_match_wickets(assisted_fielder_id);

create index if not exists idx_cricket_match_wickets_credited_bowler_id on public.cricket_match_wickets(credited_bowler_id);

create index if not exists idx_cricket_match_wickets_player_out_id on public.cricket_match_wickets(player_out_id);

create index if not exists idx_cricket_match_wickets_primary_fielder_id on public.cricket_match_wickets(primary_fielder_id);

comment on table public.cricket_match_wickets is 'CRICKET ENGINE TABLE (legacy generic name). Planned rename: '
