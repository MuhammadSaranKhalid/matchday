...
create table public.cricket_match_players(
  ...
  constraint cricket_match_players_parent_player_fkey foreign key (match_player_id, match_id) references public.match_players(match_player_id, match_id) on delete cascade,
  constraint cricket_match_players_cricket_match_fkey foreign key (match_id) references public.cricket_matches(match_id) on delete cascade
);

comment on table public.cricket_match_players is 'Cricket-only per-match player state. Shared identity remains in '
...
create trigger cricket_match_players_set_updated_at
  before update on public.cricket_match_players for each row
...
create policy "cricket_match_players_read_all" on public.cricket_match_players
...
create index cricket_match_players_match on public.cricket_match_players(match_id);
