...
create table public.posts (
  ...
  constraint post_author_context_consistency check (
...
-- Indexes
-- -----------------------------------------------------------------------------
create index posts_author_created
  on public.posts (author_id, created_at desc);
create index posts_status_created
  on public.posts (status, created_at desc)
  where status = 'active';
create index posts_linked_match
  on public.posts (linked_match_id) where linked_match_id is not null;
create index posts_linked_tournament
  on public.posts (linked_tournament_id) where linked_tournament_id is not null;
create index posts_linked_team
  on public.posts (linked_team_id) where linked_team_id is not null;
create index posts_linked_players_gin
  on public.posts using gin (linked_player_ids);
create index posts_context_entity
  on public.posts (context_entity_id) where context_entity_id is not null;
...
as $$
  select exists (
    select 1 from public.posts p
     where p.post_id = p_post_id and p.author_id = auth.uid()
  );
$$;
...
create policy "posts_update_author_or_manager"
...
create policy "posts_delete_author_or_manager"
