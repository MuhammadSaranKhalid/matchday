-- Migration file: 20260101000520_comments.sql
...
create index comments_parent on public.comments(parent_comment_id)
where
  parent_comment_id is not null;

create index comments_author on public.comments(author_id);

create index comments_mentions_gin on public.comments using gin(mentioned_user_ids);
...
create trigger comments_enforce_single_level
  before insert or update of parent_comment_id on public.comments for each row
...
create trigger comments_bump_post_count
  after insert or update of status or delete on public.comments for each row
...
create or replace function public.stamp_comment_edited_at()
  ...
  as $$
begin
  if new.text is distinct from old.text or new.mentioned_user_ids is distinct from old.mentioned_user_ids then
    new.edited_at = now();
  end if;
  return new;
end;
...
create policy "comments_read_if_post_visible" on public.comments
...
create policy "comments_delete_self_or_post_author" on public.comments
...
begin
  perform
    realtime.send(to_jsonb(new), 'comment_added', 'post:' || new.post_id::text || ':comments', true);
  return null;
end;
$$;
...
begin
  perform
    realtime.send(to_jsonb(new), 'comment_updated', 'post:' || new.post_id::text || ':comments', true);
  return null;
end;
$$;
...
create trigger comments_after_update_broadcast
  after update on public.comments for each row
  when(old.text is distinct from new.text or old.status is distinct from new.status or old.likes_count is distinct from new.likes_count or old.mentioned_user_ids is distinct from new.mentioned_user_ids or old.edited_at is distinct from new.edited_at)
...
begin
  perform
    realtime.send(jsonb_build_object('comment_id', old.comment_id, 'post_id', old.post_id, 'parent_comment_id', old.parent_comment_id), 'comment_deleted', 'post:' || old.post_id::text || ':comments', true);
  return null;
end;
$$;
