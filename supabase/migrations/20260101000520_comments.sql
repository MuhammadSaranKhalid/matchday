...
begin
  if new.text is distinct from old.text
     or new.mentioned_user_ids is distinct from old.mentioned_user_ids then
    new.edited_at = now();
  end if;
  return new;
end;
    ...
    exists (
      select 1 from public.posts p
       where p.post_id = comments.post_id
         and (p.status = 'active' or p.author_id = (select auth.uid()))
    )
  );
...
create trigger comments_after_insert_broadcast
  after insert on public.comments
  for each row execute function public.broadcast_new_comment();
...
create trigger comments_after_delete_broadcast
  ...
  for each row execute function public.broadcast_comment_deleted();
