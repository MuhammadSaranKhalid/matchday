-- =============================================================================
-- Migration: 20260101000561_get_follow_list_rpc.sql
-- =============================================================================
...
begin
  if p_direction = 'followers' then
    return query select
        ...
        case
          when v_actor is not null then exists (
            select
              1
            from public.follows
            where
      ...
      where f.target_type = 'user' and f.target_id = p_user_id
    ...
    return query select
      ...
      where f.follower_id = p_user_id and f.target_type = 'user'
      order by f.created_at desc
      limit coalesce(p_limit, 100) offset coalesce(p_offset, 0);
  end if;
end;
...
grant execute
on function public.get_follow_list(uuid, text, int, int)
