...
create or replace function public._notify_vars(
  p_payload jsonb,
  p_actor_id uuid
)
returns jsonb
...
begin
  ...
  where
    ...
    and is_active;
  if not found then
    raise exception 'notify(): unknown or inactive notification type "%"', p_type_key
      using errcode = '22023';
  end if;
  ...
  if nullif(v_collapse, '') is not null then
    for v_recipient in select distinct
      id
    from
      unnest(p_recipients) r(id)
where
  id is not null
order by
  id loop
    perform
      pg_advisory_xact_lock(hashtextextended(v_recipient::text || ':' || v_collapse, 0));
  end loop;
    update
    ...
    set
      collapse_key = null
    where
      recipient_id = any (p_recipients)
      and collapse_key = v_collapse
      and not is_read
      and collapse_until <= now();
  end if;
  ...
  with targets as (
    ...
    where
      ...
      and (p_actor_id is null
        or r.id <> p_actor_id)
      -- 2. entity mute
      and (not v_type.user_configurable
        or not exists (
          select
            1
          from
        ...
        and (not v_type.user_configurable
          or coalesce((
            ...
            where
              ...
              and pp.channel = 'inapp'), v_inapp_def))
),
ins as (
insert into public.notifications(recipient_id, type_key, title, body, route, tier, icon, icon_path, tone, actor_id, entity_scope, entity_id, payload, collapse_key, collapse_until)
  select
    t.recipient_id,
    v_type.key,
    v_title,
    ...
    nullif(v_collapse, ''),
    now() + v_type.collapse_window
  from
    targets t
  on conflict (recipient_id,
    collapse_key)
  where
    collapse_key is not null
    and is_read = false
      do update set
        ...
        body = coalesce(public.render_template(v_type.body_template_grouped, v_vars || jsonb_build_object(
              ...
              'count', public.notifications.group_count + 1, 'others', public.notifications.group_count)), excluded.body), icon = excluded.icon, icon_path = excluded.icon_path, tone = excluded.tone, tier = excluded.tier, title = excluded.title, route = excluded.route, actor_id = excluded.actor_id, payload = excluded.payload, updated_at = now()
      returning
        notification_id, recipient_id
)
  select
    array_agg(i.notification_id),
    array_agg(i.notification_id) filter (where case when v_type.user_configurable then
        coalesce(pp.enabled, v_push_def)
      else
        v_push_def
      end)
  into
  ...
  from
    ins i
  left join public.notification_preferences pp on pp.user_id = i.recipient_id
    and pp.category = v_type.category
    and pp.channel = 'push';
  if v_all_ids is null then
    return;
  end if;
insert into public.notification_deliveries(notification_id, revision, device_key, channel, status, attempts)
select
  n.notification_id,
  n.group_count,
  '',
