...
begin
  ...
  new.sport_id := coalesce(v_from_sport, new.sport_id, 'cricket');
  return new;
end;
$$;
...
begin
  if not exists(
    ...
    from
      pg_constraint
    where
      conrelid = 'public.match_challenges'::regclass
      and conname = 'match_challenges_sport_id_fkey') then
  alter table public.match_challenges
    add constraint match_challenges_sport_id_fkey foreign key(sport_id) references public.sports(sport_id) on update restrict on delete restrict;
end if;
end
$$;
...
create or replace function public._validate_team_xi(
  p_team_id uuid,
  p_xi uuid[]
)
  returns void
...
begin
  ...
  if v_to_team_xi is not null and array_length(v_to_team_xi, 1) is not null and array_length(v_to_team_xi, 1) > v_req.players_per_side then
  ...
  perform
    public._validate_team_xi(v_to_team, v_to_team_xi);
