...
begin
  ...
  set
    ...
    when 'pending' then
      'auto-expired after 48h with no response'
    when 'countered' then
      'auto-expired after 24h with no response to counter'
    else
      'auto-expired'
    end
  where ((status = 'pending'
      and ((proposal_expires_at is not null
        ...
        or (proposal_expires_at is null
...
begin
  ...
  set
    status = 'abandoned'
  where
    m.status in ('scheduled', 'rescheduled')
...
begin
  if exists(
    ...
    from
      cron.job
    where
      jobname = 'expire_stale_match_requests') then
  perform
    cron.unschedule('expire_stale_match_requests');
end if;
  perform
    cron.schedule('expire_stale_match_requests', '*/15 * * * *', $sql$select public.expire_stale_match_requests();$sql$);
  if exists(
    select
      1
    from
      cron.job
    where
      jobname = 'abandon_stale_matches') then
  perform
    cron.unschedule('abandon_stale_matches');
end if;
  perform
    cron.schedule('abandon_stale_matches', '7 * * * *', $sql$select public.abandon_stale_matches();$sql$);
end
$$;
