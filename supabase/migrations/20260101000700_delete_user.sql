-- =============================================================================
-- Migration: 20260101000700_delete_user.sql
-- =============================================================================
...
begin
  if old.claimed_by_user_id is not null or new.claimed_by_user_id is null then
    return new;
  end if;
  ...
  then
    raise exception 'Claim target profile does not exist' using errcode = 'P0002';
  end if;
  ...
  insert into public.player_sports
    (user_id, sport_id)
  values
    (new.claimed_by_user_id, new.sport_id)
    ...
    on conflict (user_id) do update
      set
        ...
        preferred_ball_types = case
          ...
          ) = 0 then excluded.preferred_ball_types
  ...
  for update loop
    ...
    if v_existing_membership is null then
      ...
      update public.team_members
      set
        in_squad = in_squad or v_tm.in_squad,
  ...
  for update loop
    ...
    from public.match_players mp
    where mp.match_id = v_mp.match_id and mp.user_id = new.claimed_by_user_id
    limit 1
    for update;
      ...
      if new.sport_id = 'cricket' then
        ...
        on conflict (match_player_id) do update
          set
            ...
            is_captain = public.cricket_match_players.is_captain or excluded.is_captain,
        ...
        set
          player_of_the_match_id = v_existing_match_player,
          updated_at = now()
  ...
  perform
    public.migrate_player_stats(new.unclaimed_id, new.claimed_by_user_id);
...
create trigger unclaimed_players_finalize_claim
  after update of claimed_by_user_id on public.unclaimed_players
  for each row
        ...
        or (
          ...
          and exists (
            select
              1
            from public.posts p
            where p.author_id = v_uid and p.post_id::text = split_part(o.name, '/', 1)
          )
        )
    )
  then
...
begin
  ...
  with
    orphaned as (
      ...
      where tm.user_id = old.user_id and tmr.role_key = 'owner' and tm.status = 'active'
    ),
    successor as (
      ...
      from
        ...
        join public.team_member_roles tmr
          on tmr.membership_id = tm.membership_id and tmr.role_key = 'manager'
    ...
    demoted as (
      ...
      using successor s
      where
        tmr.membership_id = s.membership_id
