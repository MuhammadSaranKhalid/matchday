...
create table public.chat_channels (
  ...
  constraint chat_channels_context_shape check (
    (context_type = 'none' and team_id is null and match_id is null and tournament_id is null and club_id is null)
    or (context_type = 'team' and team_id is not null and match_id is null and tournament_id is null and club_id is null)
    or (context_type = 'match' and match_id is not null and team_id is null and tournament_id is null and club_id is null)
    or (context_type = 'tournament' and tournament_id is not null and team_id is null and match_id is null and club_id is null)
    or (context_type = 'club' and club_id is not null and team_id is null and match_id is null and tournament_id is null)
  ),
  constraint direct_channel_shape check (
    kind <> 'direct'
    or (context_type = 'none' and visibility = 'private')
  )
);

create unique index chat_channels_team_purpose_unique
  on public.chat_channels(team_id, purpose)
  where team_id is not null and archived_at is null;

create unique index chat_channels_match_purpose_unique
  on public.chat_channels(match_id, purpose)
  where match_id is not null and archived_at is null;

create unique index chat_channels_tournament_purpose_unique
  on public.chat_channels(tournament_id, purpose)
  where tournament_id is not null and archived_at is null;

create unique index chat_channels_club_purpose_unique
  on public.chat_channels(club_id, purpose)
  where club_id is not null and archived_at is null;

create index chat_channels_last_message_idx
  on public.chat_channels(last_message_at desc nulls last);
