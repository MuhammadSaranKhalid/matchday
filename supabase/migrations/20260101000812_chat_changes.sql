-- Migration file: 20260101000812_chat_changes.sql
...
create table if not exists public.chat_changes(
  ...
  entity_type text not null check (entity_type in ('message', 'reaction', 'receipt', 'member', 'policy')),
...
create index if not exists chat_changes_channel_seq_idx on public.chat_changes(channel_id, change_seq);
