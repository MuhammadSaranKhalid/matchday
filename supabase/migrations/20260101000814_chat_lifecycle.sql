-- Migration file: 20260101000814_chat_lifecycle.sql
...
begin
  ...
  select
    exists (
      select
        1
      from
        public.channel_role_permissions crp
      where
        crp.role = v_role
  ...
  if p_permission in ('send_messages', 'send_media') and v_posting_mode is not null then
    if v_posting_mode = 'owner' and v_role <> 'owner' then
      return false;
    elsif v_posting_mode = 'admins'
        and v_role not in ('owner', 'admin') then
        return false;
    elsif v_posting_mode = 'moderators'
        and v_role not in ('owner', 'admin', 'moderator') then
        return false;
    end if;
  end if;
  select
    exists (
      select
        1
      from
        public.channel_member_restrictions cmr
      where
        cmr.channel_id = p_channel_id
        and cmr.user_id = p_user_id
        and cmr.permission = p_permission
        and cmr.starts_at <= now()
        and cmr.revoked_at is null
        and (cmr.expires_at is null
          or cmr.expires_at > now()))
  into
    v_is_restricted;
...
create or replace function public.get_or_create_direct_channel(
...
begin
  ...
  if exists (
    ...
    or (blocker_id = p_target_user_id
      and blocked_id = v_caller_id)) then
raise exception 'Cannot message this user'
  ...
  if v_channel_id is null then
    insert into public.chat_channels(channel_key, kind, context_type, visibility, created_by)
      values (v_channel_key, 'direct', 'none', 'private', v_caller_id)
  ...
  -- Caller is active
  insert into public.channel_members(channel_id, user_id, role, status, joined_at)
    values (v_channel_id, v_caller_id, 'member', 'active', now())
  on conflict (channel_id, user_id)
    do nothing;
  -- Target user is active or pending
  insert into public.channel_members(channel_id, user_id, role, status, invited_by, invited_at, joined_at)
    values (v_channel_id, p_target_user_id, 'member', v_target_status, v_caller_id, now(), case when v_target_status = 'active' then
        now()
      else
        null
      end)
...
begin
  ...
  if not v_can_send then
    raise exception 'Permission denied: send_messages not permitted'
      using errcode = '42501';
  end if;
  select
    media_enabled,
  ...
  if p_body is not null and length(p_body) > coalesce(v_max_len, 4000) then
    raise exception 'Message exceeds max length of %', v_max_len
      using errcode = '22023';
  end if;
  if coalesce(v_slow_mode, 0) > 0 then
    ...
    if v_last_msg_at is not null and (v_now - v_last_msg_at) <(v_slow_mode * interval '1 second') then
      ...
      if v_wait_seconds < 1 then
        v_wait_seconds := 1;
      end if;
      raise exception 'CHAT_SLOW_MODE: Please wait % seconds before sending again', v_wait_seconds
        using errcode = 'P0001', detail = jsonb_build_object('code', 'CHAT_SLOW_MODE', 'retry_after_seconds', v_wait_seconds)::text;
    end if;
  end if;
  perform
    private.guard_dm_request_limit(p_channel_id, v_actor);
  insert into public.messages(message_id, channel_id, sender_id, message_type, body, payload, reply_to_message_id, created_at, updated_at)
    values (p_message_id, p_channel_id, v_actor, p_message_type, p_body, coalesce(p_payload, '{}'::jsonb), p_reply_to_message_id, v_now, v_now)
  returning
    *
  into
    v_new_msg;
  if p_payload ? 'attachment' then
    ...
    insert into public.message_attachments(attachment_id, message_id, storage_path, mime_type, file_name, size_bytes, width, height, duration_ms, created_at)
      values (coalesce((v_att ->> 'attachment_id')::uuid, gen_random_uuid()), v_new_msg.message_id, v_att ->> 'storage_path', coalesce(v_att ->> 'mime_type', 'application/octet-stream'), v_att ->> 'file_name',(v_att ->> 'size_bytes')::bigint,(v_att ->> 'width')::integer,(v_att ->> 'height')::integer,(v_att ->> 'duration_ms')::bigint, v_now)
  ...
  update
    public.channel_members
  set
    last_read_message_seq = greatest(coalesce(last_read_message_seq, 0), v_new_msg.message_seq),
    last_read_at = v_now,
  ...
  update
    public.chat_channels
  set
    last_message_seq = v_new_msg.message_seq,
...
begin
  ...
  into
    v_updated;
  insert into public.chat_changes(channel_id, entity_type, entity_id, operation, actor_id, payload)
    values (v_channel_id, 'message', p_message_id::text, 'update', v_actor, jsonb_build_object('message_id', p_message_id, 'body', p_new_body, 'version', v_updated.version, 'edited_at', v_now));
  return v_updated;
end;
$$;
...
begin
  ...
  into
    v_updated;
  insert into public.chat_changes(channel_id, entity_type, entity_id, operation, actor_id, payload)
    values (v_channel_id, 'message', p_message_id::text, 'delete', v_actor, jsonb_build_object('message_id', p_message_id, 'version', v_updated.version, 'deleted_at', v_now));
  return v_updated;
end;
$$;
...
begin
  ...
  if not private.has_channel_permission(v_actor, v_channel_id, 'add_reactions') then
    raise exception 'Permission denied: add_reactions not permitted'
      using errcode = '42501';
  end if;
  if p_selected then
    ...
    on conflict (message_id, user_id, reaction)
      do update set
        ...
        updated_at = now();
    insert into public.chat_changes(channel_id, entity_type, entity_id, operation, actor_id, payload)
      values (v_channel_id, 'reaction', p_message_id::text, 'insert', v_actor, jsonb_build_object('message_id', p_message_id, 'reaction', p_reaction, 'user_id', v_actor));
  else
    update
    ...
    insert into public.chat_changes(channel_id, entity_type, entity_id, operation, actor_id, payload)
      values (v_channel_id, 'reaction', p_message_id::text, 'delete', v_actor, jsonb_build_object('message_id', p_message_id, 'reaction', p_reaction, 'user_id', v_actor));
  end if;
end;
$$;
...
create or replace function public.decline_channel_invite(
  p_channel_id uuid
)
  returns void
  language plpgsql
...
create or replace function public.list_my_chats()
  ...
  select
    ...
    when c.context_type = 'team' then
      t.team_name
    when c.context_type = 'tournament' then
      tourn.tournament_name
    else
      c.title
    end as title,
    case when c.kind = 'direct' then
      dm_user.profile_photo_url
    when c.context_type = 'team' then
      t.logo_url
    when c.context_type = 'tournament' then
      tourn.logo_url
...
(last_msg.sender_id =(
        select
          auth.uid())) as last_message_from_me,
...
(m.archived_at is not null) as is_archived,
    m.archived_at,
(m.notifications_muted_until is not null
      and m.notifications_muted_until > now()) as is_muted,
  ...
  from
    ...
    left join lateral(
      select
        cm_other.user_id
      from
        public.channel_members cm_other
      where
        cm_other.channel_id = c.channel_id
        and cm_other.user_id <>(
          select
            auth.uid())
          and c.kind = 'direct'
        limit 1) dm_other on true
    ...
    select
      exists(
        ...
        from
          public.follows f
        where
          f.follower_id = dm_other.user_id
          and f.target_type = 'user'
          and f.target_id =(
            select
              auth.uid())) as they_follow) tf on true
  left join lateral(
    select
      exists(
        select
        ...
        where
          f.follower_id =(
            select
              auth.uid())
            and f.target_type = 'user'
            and f.target_id = dm_other.user_id) as you_follow) yf on true
  left join lateral(
    select
      msg.body,
      msg.sender_id
...
-- 3. RLS Policies
create policy "chat_channels_read_members_or_public" on public.chat_channels
  for select to authenticated
  using (visibility = 'public'
    or public.is_chat_member(channel_id));

create policy "channel_members_read_self_or_channel" on public.channel_members
  for select to authenticated
  using (user_id =(
    select
      auth.uid())
...
create policy "channel_policies_read_members" on public.channel_policies
  for select to authenticated
  using (public.is_chat_member(channel_id)
    or exists (
      ...
      where
        cc.channel_id = channel_policies.channel_id
        and cc.visibility = 'public'));

create policy "channel_role_permissions_read_all" on public.channel_role_permissions
  for select to authenticated
  using (true);

create policy "channel_member_restrictions_read_members" on public.channel_member_restrictions
  for select to authenticated
  using (user_id =(
    select
      auth.uid())
...
create policy "channel_membership_periods_read_members" on public.channel_membership_periods
  for select to authenticated
  using (user_id =(
    select
      auth.uid())
...
create policy "messages_read_for_members" on public.messages
  for select to authenticated
  using (public.is_chat_member(channel_id));

create policy "message_attachments_read_for_members" on public.message_attachments
  for select to authenticated
  using (exists (
    select
      1
    from
      public.messages m
    where
      m.message_id = message_attachments.message_id
      and public.is_chat_member(m.channel_id)));

create policy "message_reactions_read_for_members" on public.message_reactions
  for select to authenticated
  using (exists (
    select
      1
    from
      public.messages m
    where
      m.message_id = message_reactions.message_id
      and public.is_chat_member(m.channel_id)));

create policy "message_user_state_read_self" on public.message_user_state
  for select to authenticated
  using (user_id =(
    select
      auth.uid()));

create policy "message_user_state_write_self" on public.message_user_state
...
create policy "channel_receipt_events_read_members" on public.channel_receipt_events
  for select to authenticated
  using (public.is_chat_member(channel_id));

create policy "chat_changes_read_members" on public.chat_changes
  for select to authenticated
  using (public.is_chat_member(channel_id));
...
(m.notifications_muted_until is not null
  ...
  from
    ...
    left join lateral(
      select
        cm_other.user_id,
        cm_other.status::text as dm_other_member_status
      from
        public.channel_members cm_other
      where
        cm_other.channel_id = c.channel_id
