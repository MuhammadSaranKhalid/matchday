-- =============================================================================
-- Migration: 20260101000814_chat_lifecycle.sql
-- =============================================================================
...
begin
  ...
  where channel_id = p_channel_id and user_id <> p_sender_id;
  if v_recipient_status = 'pending' then
    ...
    where channel_id = p_channel_id and sender_id = p_sender_id;
...
begin
  ...
  if
    exists (
      ...
      where
        (blocker_id = v_caller_id and blocked_id = p_target_user_id)
  ...
  insert into public.channel_members
    (channel_id, user_id, role, status, invited_by, invited_at, joined_at)
  values
    (
      v_channel_id,
      p_target_user_id,
...
begin
  ...
  where channel_id = p_channel_id;
  if
    p_message_type in ('image', 'video', 'audio', 'file')
    and not coalesce(v_media_enabled, true)
  then
  ...
  if coalesce(v_slow_mode, 0) > 0 then
    ...
    limit 1;
    if
      v_last_msg_at is not null
      and (v_now - v_last_msg_at) < (v_slow_mode * interval '1 second')
    then
      v_wait_seconds := v_slow_mode - floor(
        extract(epoch from (v_now - v_last_msg_at))
      )::integer;
  ...
  set
    last_read_message_seq = greatest(
      ...
      v_new_msg.message_seq
    ),
    last_read_at = v_now,
    last_delivered_message_seq = greatest(
...
begin
  ...
  select
    channel_id,
    version
  into v_channel_id, v_current_version
  from public.messages
  where message_id = p_message_id and sender_id = v_actor and deleted_at is null;
  if v_channel_id is null then
...
begin
  ...
  if v_sender_id = v_actor then
    if
      not private.has_channel_permission(v_actor, v_channel_id, 'delete_own_messages')
    then
    ...
    if
      not private.has_channel_permission(v_actor, v_channel_id, 'delete_any_message')
    then
...
begin
  ...
  if p_selected then
    insert into public.message_reactions
      (message_id, user_id, reaction, updated_at, removed_at)
...
begin
  ...
  into v_delivery_receipts_enabled
  from public.channel_policies
  where channel_id = p_channel_id;
  update public.channel_members
  set
...
begin
  ...
  if v_context_type in ('team', 'match', 'tournament') then
    raise exception 'CHAT_CANNOT_LEAVE_ENTITY_CHANNEL: Cannot leave % chat directly. Manage notification preferences or entity membership instead.',
      v_context_type
      using errcode = 'P0001';
  end if;
  update public.channel_members
  ...
  where channel_id = p_channel_id and user_id = v_actor and left_at is null;
end;
$$;
...
as $$
  select
    ...
    (
      ...
      and m.notifications_muted_until > now()
    ) as is_muted,
  ...
  from
    ...
    left join lateral (
      select
        ...
        msg.sender_id
      from public.messages msg
      where msg.channel_id = c.channel_id and msg.deleted_at is null
...
create policy "channel_policies_read_members"
  ...
  using (
    ...
    or exists (
      ...
      where cc.channel_id = channel_policies.channel_id and cc.visibility = 'public'
    )
  );
...
create policy "channel_member_restrictions_read_members"
  on public.channel_member_restrictions
  for select
  to authenticated
  using (
    user_id = (
...
create policy "channel_membership_periods_read_members"
  on public.channel_membership_periods
  for select
  to authenticated
  using (
    user_id = (
...
create policy "channel_receipt_events_read_members"
  on public.channel_receipt_events
  for select
  to authenticated
  using (public.is_chat_member(channel_id));
