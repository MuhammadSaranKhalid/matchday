-- Migration file: 20260101000815_chat_realtime_broadcast.sql
...
begin
  ...
  v_payload := jsonb_build_object('message_id', new.message_id, 'channel_id', new.channel_id, 'sender_id', new.sender_id, 'sender_name', v_sender_name, 'sender_avatar_url', v_sender_avatar, 'sender_username', v_sender_username, 'message_type', new.message_type, 'body', new.body, 'payload', new.payload, 'reply_to_message_id', new.reply_to_message_id, 'message_seq', new.message_seq, 'version', new.version, 'created_at', new.created_at);
  perform
    net.http_post(url := 'https://rest.ably.io/channels/chat:' || new.channel_id || '/messages', headers := jsonb_build_object('Authorization', v_auth_header, 'Content-Type', 'application/json'), body := jsonb_build_object('name', 'message.created', 'data', v_payload::text));
  for v_member in
  select
    user_id
  from
  ...
  where
    ...
    loop
      perform
        net.http_post(url := 'https://rest.ably.io/channels/user:' || v_member.user_id || '/messages', headers := jsonb_build_object('Authorization', v_auth_header, 'Content-Type', 'application/json'), body := jsonb_build_object('name', 'channel.updated', 'data', jsonb_build_object('channel_id', new.channel_id, 'last_message_seq', new.message_seq, 'last_message_body', new.body, 'last_message_at', new.created_at, 'sender_id', new.sender_id, 'sender_name', v_sender_name)::text));
    end loop;
  return new;
end;
$$;
...
create trigger trg_broadcast_message_mutation_to_ably
  after update on public.messages for each row
...
begin
  ...
  v_selected :=(tg_op = 'INSERT'
    or (tg_op = 'UPDATE'
      and new.removed_at is null));
  v_payload := jsonb_build_object('message_id', coalesce(new.message_id, old.message_id), 'user_id', coalesce(new.user_id, old.user_id), 'reaction', coalesce(new.reaction, old.reaction), 'selected', v_selected, 'updated_at', coalesce(new.updated_at, old.updated_at));
  perform
    net.http_post(url := 'https://rest.ably.io/channels/chat:' || v_channel_id || '/messages', headers := jsonb_build_object('Authorization', v_auth_header, 'Content-Type', 'application/json'), body := jsonb_build_object('name', 'reaction.updated', 'data', v_payload::text));
  return coalesce(new, old);
end;
$$;
...
create trigger trg_broadcast_reaction_to_ably
  after insert or update on public.message_reactions for each row
...
begin
  ...
  if (old.last_read_message_seq is distinct from new.last_read_message_seq) and new.last_read_message_seq is not null then
    perform
      net.http_post(url := 'https://rest.ably.io/channels/chat:' || new.channel_id || '/messages', headers := jsonb_build_object('Authorization', v_auth_header, 'Content-Type', 'application/json'), body := jsonb_build_object('name', 'receipt.read', 'data', jsonb_build_object('channel_id', new.channel_id, 'user_id', new.user_id, 'through_seq', new.last_read_message_seq, 'read_at', new.last_read_at)::text));
  end if;
  if (old.last_delivered_message_seq is distinct from new.last_delivered_message_seq) and new.last_delivered_message_seq is not null then
    perform
      net.http_post(url := 'https://rest.ably.io/channels/chat:' || new.channel_id || '/messages', headers := jsonb_build_object('Authorization', v_auth_header, 'Content-Type', 'application/json'), body := jsonb_build_object('name', 'receipt.delivered', 'data', jsonb_build_object('channel_id', new.channel_id, 'user_id', new.user_id, 'through_seq', new.last_delivered_message_seq, 'delivered_at', new.last_delivered_at)::text));
  end if;
  return new;
end;
$$;
...
create trigger trg_broadcast_receipt_to_ably
  after update on public.channel_members for each row
...
begin
  ...
  perform
    net.http_post(url := rtrim(v_url, '/') || '/functions/v1/send-chat-push', headers := jsonb_build_object('content-type', 'application/json', 'x-worker-secret', v_key), body := jsonb_build_object('message_id', new.message_id), timeout_milliseconds := 15000);
  return new;
exception
  when others then
