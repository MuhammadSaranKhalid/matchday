-- =============================================================================
-- Migration: 20260101000816_chat_domain_sync.sql
-- =============================================================================
...
begin
  ...
  where match_id = v_match_id and context_type = 'match' and purpose = 'main'
  limit 1;
  ...
  if v_is_active then
    ...
    set
      ...
      updated_at = clock_timestamp()
    where channel_id = v_channel_id and user_id = v_user_id and status = 'active';
  end if;
  return coalesce(new, old);
end;
$$;
...
begin
  ...
  where team_id = new.team_id and status = 'active' and user_id is not null loop
    insert into public.channel_members (channel_id, user_id, role, status, joined_at)
...
begin
  ...
  on conflict (team_id, purpose) where
    team_id is not null
    and archived_at is null do nothing
...
begin
  ...
  insert into public.chat_channels
    (channel_key, kind, context_type, visibility, purpose, match_id, created_by)
  values (v_key, 'group', 'match', 'private', 'main', new.match_id, new.created_by)
  on conflict (match_id, purpose) where
...
begin
  ...
  insert into public.chat_channels
    (channel_key, kind, context_type, visibility, purpose, tournament_id, created_by)
  values
    (v_key, 'group', 'tournament', 'private', 'main', new.tournament_id, new.created_by)
  on conflict (tournament_id, purpose) where
...
where
  ...
  and not exists (
    select
      1
    from public.channel_policies p
