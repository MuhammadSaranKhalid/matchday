-- Migration file: 20260101000816_chat_domain_sync.sql
...
begin
  if(tg_op = 'INSERT' and new.status = 'active') or(tg_op = 'UPDATE' and old.status <> 'active' and new.status = 'active') then
    insert into public.channel_membership_periods(channel_id, user_id, joined_at)
      values(new.channel_id, new.user_id, coalesce(new.joined_at, clock_timestamp()))
  ...
  where
    left_at is null
      ...
      set
        left_at = coalesce(new.left_at, clock_timestamp()),
...
create trigger trg_sync_channel_membership_period
  after insert or update on public.channel_members for each row
  execute function public.sync_channel_membership_period();
...
create trigger trg_sync_channel_last_message
  ...
  execute function public.sync_channel_last_message();
...
begin
  ...
  if v_is_active then
    insert into public.channel_members(channel_id, user_id, role, status, joined_at)
      values (v_channel_id, v_user_id, 'member', 'active', clock_timestamp())
    on conflict (channel_id, user_id)
      do update set
        status = 'active',
        left_at = null,
...
create trigger trg_sync_match_player_chat
  after insert or update or delete on public.match_players for each row
  execute function public.sync_match_player_chat();
...
create trigger trg_sync_team_member_chat
  after insert or update on public.team_members for each row
  execute function public.sync_team_member_chat();
...
create trigger trg_sync_tournament_team_chat
  after insert or update on public.tournament_teams for each row
  execute function public.sync_tournament_team_chat();
...
begin
  ...
  insert into public.chat_channels(channel_key, kind, context_type, visibility, purpose, team_id, created_by)
    values (v_key, 'group', 'team', 'private', 'main', new.team_id, new.created_by)
  on conflict (team_id, purpose)
where
  team_id is not null
  ...
  if v_channel_id is not null then
    ...
    if new.created_by is not null then
      insert into public.channel_members(channel_id, user_id, role, status, joined_at)
        values (v_channel_id, new.created_by, 'owner', 'active', clock_timestamp())
      on conflict (channel_id, user_id)
        do nothing;
    end if;
  end if;
...
create trigger teams_after_insert_create_chat
  ...
  execute function public.create_team_chat();
...
begin
  ...
  insert into public.chat_channels(channel_key, kind, context_type, visibility, purpose, match_id, created_by)
    values (v_key, 'group', 'match', 'private', 'main', new.match_id, new.created_by)
  on conflict (match_id, purpose)
where
...
begin
  ...
  insert into public.chat_channels(channel_key, kind, context_type, visibility, purpose, tournament_id, created_by)
    values (v_key, 'group', 'tournament', 'private', 'main', new.tournament_id, new.created_by)
  on conflict (tournament_id, purpose)
where
...
create trigger tournaments_after_insert_create_chat
  after insert on public.tournaments for each row
  execute function public.create_tournament_chat();
