-- Migration file: 20260101000850_content_reports.sql
...
create index content_reports_open on public.content_reports(created_at)
where
  status = 'open';
    ...
    when 'message' then
      exists(
        ...
        from
          public.messages
        where
          message_id = new.target_id)
    else
      false
    end) then
raise exception 'Content is no longer available'
      using errcode = '42501';
  end if;
  return new;
end;
...
create trigger validate_content_report
  before insert on public.content_reports for each row
