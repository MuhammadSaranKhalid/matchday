-- Migration file: 20260101000910_notification_delivery_queue.sql
...
begin
  perform
    pgmq.drop_queue('notifications_push');
  perform
    pgmq.drop_queue('notifications_push_bulk');
exception
  when others then
    null;
end
...
select
  pgmq.create('notifications_push');

select
  pgmq.create('notifications_push_bulk');

-- Section: Functions

create or replace function public._notify_deliver(
...
begin
  ...
  select
    array_agg(jsonb_build_object('notification_id', n.notification_id, 'revision', n.group_count, 'token_id', d.token_id, 'recipient_id', n.recipient_id, 'title', n.title, 'body', n.body, 'route', n.route, 'type_key', n.type_key, 'importance', t.importance))
  into
    v_jobs
  from
  ...
  where
    n.notification_id = any (p_notification_ids);
  if v_jobs is not null then
    perform
      pgmq.send_batch(p_queue_name, v_jobs);
  end if;
  insert into public.notification_deliveries(notification_id, revision, device_key, status)
  select
    n.notification_id,
    n.group_count,
    '',
...
create or replace function public.read_notification_jobs(
  p_queue text
)
  returns jsonb
  language plpgsql
...
begin
  ...
  v_limit := case when p_queue = 'notifications_push' then
    50
  else
    100
  end;
  select
    coalesce(jsonb_agg(jsonb_build_object('msg_id', r.msg_id::text, 'read_ct', r.read_ct, 'message', r.message)), '[]'::jsonb)
  into
    v_jobs
  from
    pgmq.read(p_queue, 120, v_limit) r;
  return v_jobs;
end;
$$;
...
create or replace function public.notification_push_status(
  p_id uuid,
  p_revision int
)
  returns text
  ...
  as $$
  select
    case when n.group_count <> p_revision then
      'superseded'
    when not t.user_configurable then
      null
    when exists(
      select
        1
      ...
      where
        m.user_id = n.recipient_id
        and m.scope = n.entity_scope
    ...
    when not coalesce((
      ...
      where
        p.user_id = n.recipient_id
        and p.category = t.category
        and p.channel = 'inapp'), 'inapp' = any(t.default_channels)) then
      'skipped_pref'
    when not coalesce((
      select
        enabled
    ...
    where
      p.user_id = n.recipient_id
      and p.category = t.category
      and p.channel = 'push'), 'push' = any(t.default_channels)) then
      'skipped_pref'
    else
      null
    end
...
begin
  ...
  perform
    net.http_post(url := rtrim(v_url, '/') || '/functions/v1/send-push', headers := jsonb_build_object('content-type', 'application/json', 'x-worker-secret', v_key), body := '{}'::jsonb, timeout_milliseconds := 55000);
end;
$$;
...
begin
  if exists(
    ...
    from
      cron.job
    where
      jobname = 'notification-push-worker') then
  perform
    cron.unschedule('notification-push-worker');
end if;
  perform
    cron.schedule('notification-push-worker', '* * * * *', $sql$select public.wake_notification_worker();$sql$);
end
$$;
