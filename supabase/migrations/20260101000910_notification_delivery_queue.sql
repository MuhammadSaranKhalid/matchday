-- =============================================================================
-- Migration: 20260101000910_notification_delivery_queue.sql
-- =============================================================================
...
revoke all
on function public._notify_deliver(uuid[], text)
...
begin
  ...
  v_limit := case
    when p_queue = 'notifications_push' then 50
    else 100
  end;
  select
    coalesce(
...
revoke all
on function public.read_notification_jobs(text)
from public, anon, authenticated;
...
revoke all
on function public.finish_notification_job(text, bigint)
from public, anon, authenticated;
...
revoke all
on function public.notification_push_status(uuid, int)
from public, anon, authenticated;
...
revoke all
on function public.wake_notification_worker()
from public, anon, authenticated;
