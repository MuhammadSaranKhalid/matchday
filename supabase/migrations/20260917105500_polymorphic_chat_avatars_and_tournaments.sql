-- 20260917105500 · Polymorphic Chat Avatars, Tournaments, and Monograms
-- Extends list_my_chats to resolve tournament metadata, dynamic team/user logos, and fallbacks.
...
begin
  ...
  select
    ...
    case
      when cc.context_type = 'team' then t.logo_url
      when cc.context_type = 'tournament' then tour.logo_url
      else cc.avatar_url
    end as team_logo_url,
    case
      when cc.context_type = 'team' then coalesce(t.logo_monogram, substring(t.team_name from 1 for 2))
      when cc.context_type = 'tournament' then substring(tour.tournament_name from 1 for 2)
      when cc.context_type = 'match' then 'VS'
      else null
    end as team_logo_monogram,
    case
      when cc.context_type = 'team' then (t.team_colors->>'primary')
      when cc.context_type = 'tournament' then '#D97706'
      else null
    end as team_primary_color,
  ...
  left join public.tournaments tour on tour.tournament_id = cc.tournament_id and cc.context_type = 'tournament'
